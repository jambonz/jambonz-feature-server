...
Duration=${duration} `
 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
    ...
    try {
      ...
      if (this.direction === CallDirection.Inbound) {
        if (task.earlyMedia && !this.req.finalResponseSent) {
          // send 180 here can cause some client play ringing tone,
          // that will cause early media is mixed with ringback tone
          // 100 Trying already sent, so we can skip 180 Ringing
          if (!task.canDelayEarlyMedia) {
  ...
  async replyEarlyMedia() {
    if (this.ep && !this.req.finalResponseSent) {
      this.res.send(183, {body: this.ep.local.sdp});
    }
  }
