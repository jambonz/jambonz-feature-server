...
class CallSession extends Emitter {
  ...
  constructor({logger, application, srf, tasks, callInfo, accountInfo, rootSpan, memberId, confName, confUuid}) {
    ...
    this.on('transcribe-start', () => {
      ...
      this.logger.debug(`CallSession:transcribe-start - stt_start_time set to ${this.stt_start_time}`);
    });
    this.on('on-transcription', () => {
      this.stt_on_transcription_time = Date.now();
      this.logger.debug(
        `CallSession:transcribe-on - stt_on_transcription_time set to ${this.stt_on_transcription_time}`);
    });
    this.on('transcribe-stop', () => {
      this.stt_stop_time = Date.now();
      this.logger.debug(`CallSession:transcribe-stop - stt_end_time set to ${this.stt_stop_time}`);
    });
  }

  get sttLatencyCalculationEnabled() {
    return this._sttLatencyCalculationEnabled || false;
  }

  set sttLatencyCalculationEnabled(enabled) {
    this._sttLatencyCalculationEnabled = enabled;
  }
...
Duration=${duration} `
 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  startSttLatencyVad() {
    if (this.sttLatencyCalculationEnabled) {
