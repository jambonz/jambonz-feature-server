...
class CallSession extends Emitter {
  ...
  constructor({logger, application, srf, tasks, callInfo, accountInfo, rootSpan, memberId, confName, confUuid}) {
    ...
    /* a call transferred from another feature server is still being recorded by the SBC */
    this._recordState = application?.siprecRecording?.state || RecordState.RecordingOff;
    if (application?.siprecRecording?.options) {
      this.recordOptions = application.siprecRecording.options;
    }
