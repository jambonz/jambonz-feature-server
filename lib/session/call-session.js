...
class CallSession extends Emitter {
  ...
  constructor({logger, application, srf, tasks, callInfo, accountInfo, rootSpan, memberId, confName, confUuid}) {
    ...
    this.on('transcribe-start', () => {
      this.sttLatencyCalculator.sttStartTime = Date.now();
      this.sttLatencyCalculator.sttStopTime = 0;
      this.sttLatencyCalculator.sttOnTranscriptionTime = 0;
      this.sttLatencyCalculator.clearTalkspurts();
      this.logger.debug(
        `CallSession:transcribe-start - stt_start_time set to ${this.sttLatencyCalculator.sttStartTime}`);
    });
    this.on('on-transcription', () => {
      this.sttLatencyCalculator.sttOnTranscriptionTime = Date.now();
      this.logger.debug(
        `CallSession:transcribe-on - 
        stt_on_transcription_time set to ${this.sttLatencyCalculator.sttOnTranscriptionTime}`);
    });
    this.on('transcribe-stop', () => {
      this.sttLatencyCalculator.sttStopTime = Date.now();
      this.logger.debug(`CallSession:transcribe-stop - stt_end_time set to ${this.sttLatencyCalculator.sttStopTime}`);
    });
  }
