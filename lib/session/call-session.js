  ...
  _preCacheAudio(newTasks) {
    ...
    for (const task of tasks) {
      ...
      if (task.name === TaskName.Say) {
        ...
        let model;
        if (vendor === 'elevenlabs' && task.synthesizer?.options?.model_id) {
          model = task.synthesizer.options.model_id;
        }

        const { span, ctx } = this.rootSpan.startChildSpan(`verb:precache-${task.summary}`);
        span.setAttributes({ 'verb.summary': task.summary });
        task.span = span;
        task.ctx = ctx;
        ...
        task._synthesizeWithSpecificVendor(this, this.ep, { vendor, language, voice, label, preCache: true, model })
          .catch((err) => {
            this.logger.error(err, 'CallSession:_preCacheAudio - error precaching audio');
          })
          .finally(() => {
            task.span.end();
          });
