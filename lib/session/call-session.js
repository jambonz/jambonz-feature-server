    ...
    if (this.accountInfo.speech && this.accountInfo.speech.length > 0) {
      ...
      if (credential && (
        ...
        if ('google' === vendor) {
          ...
          this.logger.info({credential}, 'CallSession:getSpeechCredentials - openai credential');
          return {
            api_key: credential.api_key
          };
        }
...
Duration=${duration} `
 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  _onUserSaid(transcript) {
    ...
    this.logger.debug({turns: this.conversationTurns}, 'CallSession:_onUserSaid');
  }
  ...
  _onBotSaid(transcript) {
    ...
    this.logger.debug({turns: this.conversationTurns}, 'CallSession:_onBotSaid');
  }
