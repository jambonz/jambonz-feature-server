 ...
 */
    ...
    while (this.tasks.length && !this.callGone) {
      ...
      if (0 === this.tasks.length && this.hasStableDialog && this.requestor instanceof WsRequestor) {
        ...
        try {
          ...
          const {reason, command} = await this._awaitCommandsOrHangup();
          span.setAttributes({completion: reason});
          if (command) span.setAttributes({command});
          span.end();
  ...
  */
  ...
  _onCommand({msgid, command, call_sid, queueCommand, data}) {
    ...
    const cmd = {queue: queueCommand, command};
    switch (command) {
      case 'redirect':
    ...
    if (this.wakeupResolver) {
      ...
      this.wakeupResolver({reason: 'received command', command: cmd});
      this.wakeupResolver = null;
    }
  }
  ...
  _clearResources() {
    ...
    this.rootSpan && this.rootSpan.end(); // otel
    this.dlg = null;
    this.ep = null;
