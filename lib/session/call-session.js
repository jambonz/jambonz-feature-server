 ...
 */
  ...
  get sipIndialogActionHook() {
    return this._sipIndialogActionHook;
  }

  set sipIndialogActionHook(url) {
    this._sipIndialogActionHook = url;
  }
  ...
  */
  ...
  async propagateAnswer() {
    if (!this.dlg) {
      ...
      if (this.sipIndialogActionHook) {
        this.dlg.on('info', this._onIndilogRequest.bind(this));
        this.dlg.on('message', this._onIndilogRequest.bind(this));
      }
      this.logger.debug(`CallSession:propagateAnswer - answered callSid ${this.callSid}`);
    }
  }

  async _onIndilogRequest(req, res) {
    if (!this.sipIndialogActionHook) {
      return;
    }
    ...
    if (sip_method === 'INFO') {
      ...
      this.logger.info(`CallSession:_onIndilogRequest unsported method: ${req.method}`);
      res.send(501);
      return;
    }
    const params = {sip_method, sip_body: req.body};
    this.currentTask.performHook(this, this.sipIndialogActionHook, params);
  }
