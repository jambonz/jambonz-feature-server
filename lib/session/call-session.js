 ...
 */
  ...
  async startBackgroundListen(opts) {
    ...
    try {
      ...
      const childSpan = this.rootSpan.startChildSpan(`background-gather:${this.backgroundListenTask.summary}`);
      this.backgroundListenTask.span = childSpan;
  ...
  async enableBotMode(gather, autoEnable) {
    try {
      ...
      const childSpan = this.rootSpan.startChildSpan(`background-gather:${this.backgroundGatherTask.summary}`);
      this.backgroundGatherTask.span = childSpan;
  ...
  async exec() {
    ...
    while (this.tasks.length && !this.callGone) {
      ...
      try {
        ...
        if (!skip) {
          task.span = this.rootSpan.startChildSpan(`verb:${task.summary}`);
      ...
      if (0 === this.tasks.length && this.requestor instanceof WsRequestor && !this.callGone) {
        const childSpan = this.rootSpan.startChildSpan('waiting for commands');
        try {
          ...
          childSpan.endWithError('error waiting for new commands');
  ...
  */
  ...
  _onCommand({msgid, command, call_sid, queueCommand, data}) {
    ...
    if (this.wakeupResolver) {
      ...
      this.rootSpan.startChildSpan('async command', {
        'async.request.queue': queue,
        'async.request.command': command
      }).end();
    }
    ...
    if (!this.dlg) {
      ...
      this.dlg = await this.srf.createUAS(this.req, this.res, {
        headers: {
          ...
          ...this.rootSpan.getSIPTracingPropagationHeaders(),
        },
  ...
  async _notifyCallStatusChange({callStatus, sipStatus, sipReason, duration}) {
    ...
    const {customerData, ...otherData} = this.callInfo;
    const childSpan = this.rootSpan.startChildSpan(`call-status:${this.callInfo.callStatus}`, {
      ...otherData,
