  ...
  set actionHookDelayProperties(opts) {
    if (this._actionHookDelayProcessor) {
      ...
      this._actionHookDelayProcessor.on('giveup', () => {
        this.logger.info('CallSession: ActionHookDelayProcessor: giveup event');
        this._jambonzHangup();
      });
    }
  }

  clearOrRestoreActionHookDelayProcessor() {
    if (this._actionHookDelayProcessor) {
      this._actionHookDelayProcessor.stop();
      if (!this.popActionHookDelayProperties()) {
        this.logger.debug('CallSession:clearOrRestoreActionHookDelayProcessor - clear ahd');
        this.clearActionHookDelayProcessor();
      }
      else {
        this.logger.debug('CallSession:clearOrRestoreActionHookDelayProcessor - restore ahd');
      }
    }
  }

  clearActionHookDelayProcessor() {
    if (this._actionHookDelayProcessor) {
      this._actionHookDelayProcessor.stop();
      this._actionHookDelayProcessor.removeAllListeners();
      this._actionHookDelayProcessor = null;
    ...
    while (this.tasks.length && !this.callGone) {
      ...
      if (0 === this.tasks.length &&
        ...
        try {
          ...
          this.clearOrRestoreActionHookDelayProcessor();
