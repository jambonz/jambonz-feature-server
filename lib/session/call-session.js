...
Duration=${duration} `
 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  _preCacheAudio(newTasks) {
    ...
    const tasks = newTasks.slice(haveQueuedNonConfig ? 0 : 1);
    const idxFirstNotConfig = tasks.findIndex((t) => t.name !== TaskName.Config);
    if (-1 === idxFirstNotConfig) return;

    const allTasksAfterFirstNotConfig = tasks.slice(idxFirstNotConfig);
