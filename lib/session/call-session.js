 ...
 */
  ...
  */
  ...
  /**
   * perform live call control - send inband DTMF
   * @param {obj} opts
   * @param {string} opts.inband_dtmf.digit - DTMF digit
   * @param {string} opts.inband_dtmf.duration - Optional, Duration
   */
  async _lccInbandDtmf(opts, callSid) {
    const {inband_dtmf} = opts;
    const {digit, duration = 100} = inband_dtmf;
    if (!this.hasStableDialog) {
      this.logger.info('CallSession:_lccInbandDtmf - invalid command as we do not have a stable call');
      return;
    }
    try {
      const dlg = callSid === this.callSid ? this.dlg : this.currentTask.dlg;
...
Duration=${duration} `
      ...
      this.logger.debug({res}, `CallSession:_lccInbandDtmf
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
      return res;
    } catch (err) {
      this.logger.error({err}, 'CallSession:_lccInbandDtmf - error sending INFO inband DTMF');
    }
  }
  ...
  async updateCall(opts, callSid) {
    ...
    } else if (opts.inband_dtmf) {
      await this._lccInbandDtmf(opts, callSid);
    }
    else if (opts.record) {
  ...
  _onCommand({msgid, command, call_sid, queueCommand, data}) {
    ...
    switch (command) {
      ...
      case 'dtmf:inband':
        this._lccInbandDtmf(data, call_sid)
          .catch((err) => {
            this.logger.info({err, data}, `CallSession:_onCommand - error sending inband DTMF ${data}`);
          });
        break;
