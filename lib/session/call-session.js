 ...
 */
  ...
  get isBotModeEnabled() {
    return this.backgroundTaskManager.isTaskRunning('gather');
  }
  ...
  async enableBotMode(gather, autoEnable) {
    try {
      ...
      if (this.isBotModeEnabled) {
        task = this.backgroundTaskManager.getTask('gather');
      ...
      task = await this.backgroundTaskManager.newTask('gather', gather);
      task.sticky = autoEnable;
      task.once('gather-done', () => {
  ...
  async disableBotMode() {
    this.backgroundTaskManager.stop('gather');
  }
