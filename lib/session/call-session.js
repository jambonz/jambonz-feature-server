    ...
    while (this.tasks.length && !this.callGone) {
      ...
      if (0 === this.tasks.length &&
        ...
        try {
          ...
          this.logger.debug('CallSession:exec _awaitCommandsOrHangup returned');
...
Duration=${duration} `
 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  replaceApplication(tasks) {
    ...
    if (this.currentTask) {
      this.logger.debug(`CallSession:replaceApplication - killing current task: ${this.currentTask.summary}`);
  ...
  kill(onBackgroundGatherBargein = false) {
    ...
    if (onBackgroundGatherBargein) {
      ...
      this.logger.debug(`CallSession:kill - current stack depth is ${this.tasks.length}`);
