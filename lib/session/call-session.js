 ...
 */
  ...
  */
  ...
  async reAnchorMediaForReInvite(res) {
    assert(this.dlg && this.dlg.connected && !this.ep);
    this.ep = await this.ms.createEndpoint({remoteSdp: this.dlg.remote.sdp});
    this._configMsEndpoint();
    res.send(200, {body: this.ep.local.sdp});
  ...
  async releaseMediaToSBCForReInvite(res, remoteSdp) {
    assert(this.dlg && this.dlg.connected && this.ep && typeof remoteSdp === 'string');

    this.ep.destroy()
      .then(() => this.ep = null)
      .catch((err) => this.logger.error({err}, 'callSession:releaseMediaToSBCForReInvite: Error destroying endpoint'));
