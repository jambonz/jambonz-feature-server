...
Duration=${duration} `
 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  async reAnchorMedia(currentMediaRoute = MediaPath.PartialMedia) {
    ...
    if (currentMediaRoute === MediaPath.PartialMedia) {
      this.logger.debug('CallSession:reAnchorMedia: re-anchoring media after partial media');
      this.ep = await this.ms.createEndpoint({remoteSdp: this.dlg.remote.sdp});
      this._configMsEndpoint();
      await this.dlg.modify(this.ep.local.sdp, {
        headers: {
    ...
    else if (currentMediaRoute === MediaPath.NoMedia) {
      this.logger.debug('CallSession:reAnchorMedia - re-anchoring media after no media');
