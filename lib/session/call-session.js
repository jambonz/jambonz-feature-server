...
const { normalizeJambones, validate } = require('@jambonz/verb-specifications');
 ...
 */
  ...
  async _lccCallDial(opts) {
    try {
      ...
      await this._validateCreateCall(opts);
      const resp = bent('POST', 201)(restDialUrl, opts);
  ...
  async _validateCreateCall(opts) {
    ...
    if (opts.application_sid) {
      ...
      delete opts.application_sid;
      if (!opts.speech_synthesis_vendor ||
        !opts.speech_synthesis_language ||
        !opts.speech_synthesis_voice ||
        !opts.speech_recognizer_vendor ||
        !opts.speech_recognizer_language)
        throw new Error('either application_sid or set of' +
    ...
    if (!opts.call_hook && !opts.application_sid) {
      throw new Error('either call_hook or application_sid required');
    }
    ...
    if (typeof opts.call_hook === 'object' && typeof opts.call_hook.url != 'string') {
      throw new Error('call_hook must be string or an object containing a url property');
    }
    if (typeof opts.call_status_hook === 'object' && typeof opts.call_status_hook.url != 'string') {
      throw new Error('call_status_hook must be string or an object containing a url property');
    }
    if (opts.call_hook && !/^https?:/.test(opts.call_hook.url) && !/^wss?:/.test(opts.call_hook.url)) {
      throw new Error('call_hook url be an absolute url');
    }
    if (opts.call_status_hook &&
      !/^https?:/.test(opts.call_status_hook.url) &&
      !/^wss?:/.test(opts.call_status_hook.url)) {
      throw new Error('call_status_hook url be an absolute url');
    }
  }
