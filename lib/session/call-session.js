 ...
 */
  ...
  async exec() {
    ...
    while (this.tasks.length && !this.callGone) {
      ...
      this.isCurTaskTranscribe = TaskName.Transcribe === task.name || task.data?.transcribe !== undefined;
 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  _configMsEndpoint() {
    ...
    if (JAMBONES_TRANSCRIBE_EP_DESTROY_DELAY_MS) {
      configureEP(this.ep, this, this.logger, JAMBONES_TRANSCRIBE_EP_DESTROY_DELAY_MS);
    }
  }
