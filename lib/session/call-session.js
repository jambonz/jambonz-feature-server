 ...
 */
  ...
  */
    ...
    try {
      if (this.ep) {
        if (this.isSipRecCallSession) {
          ...
          if (this.onHoldMusic && this.currentTask && this.currentTask.name === TaskName.Dial) {
            this.logger.info('handling reINVITE after media has been released for music onhold');
  ...
  async reAnchorMediaForResponse(res) {
    assert(this.dlg && this.dlg.connected && !this.ep);
    // open Normal endpoint
    const sdp = this.dlg.remote.sdp.replace(/sendonly/, 'sendrecv')
    this.ep = await this.ms.createEndpoint({remoteSdp: sdp});
    // modify endpoint to real sdp
    const newSdp = await this.ep.modify(this.dlg.remote.sdp);
    res.send(200, {body: newSdp});
  }

  async releaseMediaToSBCForResponse(res, remoteSdp) {
    assert(this.dlg && this.dlg.connected && this.ep && typeof remoteSdp === 'string');

    this.ep.destroy()
      .then(() => this.ep = null)
      .catch((err) => this.logger.error({err}, 'callSession:releaseMediaToSBCForResponse: Error destroying endpoint'));

    res.send(200, {body: remoteSdp});
  }
