...
class BackgroundTaskManager extends Emitter {
  ...
  async newTask(type, taskOpts, autorestart = false) {
    ...
    switch (type) {
      case 'listen':
        ...
        break;
      case 'bargeIn':
        task = await this._initBargeIn(taskOpts, autorestart);
        break;
      case 'record':
  ...
  // Initiate Gather
  async _initBargeIn(opts, autorestart = false) {
    let task;
    try {
      const t = normalizeJambones(this.logger, [opts]);
      task = makeTask(this.logger, t[0]);
      ...
      task.exec(this.cs, resources)
        .then(() => {
          ...
          if (autorestart && !this.cs.callGone && !this.cs._stopping && this.cs._bargeInEnabled) {
            this.logger.info('BackgroundTaskManager:_initBargeIn: restarting background bargeIn');
            setImmediate(() => this._initBargeIn(opts, autorestart));
          }
          return;
        })
