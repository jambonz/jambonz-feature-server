...
class CallInfo {
   ...
   * from an earlier one.
   *
   * Absence is stored as '' rather than undefined on purpose.  toJSON() omits falsy values,
   * so the webhook payload is unaffected either way - but the same object is written to the
   * redis call record with hmset, which MERGES: an undefined value is filtered out and would
   * leave a stale header from an earlier status change readable via GET /Calls/:sid.  An
   * empty string overwrites it (same idiom as _persistConferenceState).
   */
