...
class InboundCallSession extends CallSession {
  constructor(req, res) {
    ...
    /* Don't report Trying after the caller has already given up: _onCancel above has
       reported the terminal no-answer/487, and this notification would land on top of it.
       The webhooks are merely out of order, but the redis call record is written with
       hmset - a MERGE - so the later write wins, and a sipStatus of 100 additionally
       re-adds the call to the active-call sorted set and swaps its 1-hour
       post-completion TTL for the 12-hour in-flight one.  The net effect is that a call
       the caller abandoned before we even finished fetching the application shows up as
       live in GET /Calls for the next 12 hours. */
    if (!this.req.locals.canceled) {
