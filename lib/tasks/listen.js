  ...
  async exec(cs, {ep}) {
    ...
    try {
      ...
      if (this.transcribeTask) {
        ...
        this.transcribeTask.span = this.span.startChildSpan(`nested:${this.transcribeTask.summary}`);
        this.transcribeTask.exec(cs, {ep})
          .then((result) => this.transcribeTask.span.end())
          .catch((err) => this.transcribeTask.span.endWithError('Error in transcribe task'));
      }
