...
const MAX_PLAY_AUDIO_QUEUE_SIXE = 10;
...
class TaskListen extends Task {
  ...
  async _onPlayAudio(ep, evt) {
    ...
    if (this.playAudioQueue.length <= MAX_PLAY_AUDIO_QUEUE_SIXE) {
      this.playAudioQueue.push(evt);
    }
    try {
      while (this.playAudioQueue.length > 0) {
        const e = this.playAudioQueue.shift();
        const results = await ep.play(e.file);
        this.logger.debug(`Finished playing file, result: ${JSON.stringify(results)}`);
        ep.forkAudioSendText({type: 'playDone', data: Object.assign({id: e.id}, results)});
      }
    }
    catch (err) {
