...
class SttTask extends Task {
  ...
  async compileHintsForCobalt(model, hints, opts) {
    /**
     * @param {string[]} hints - array of strings to be compiled
     * @param {object} opts - options to send to cobalt
     * 
     * @description - compiles hints for cobalt
     * (1) check to see if we already have a cached token for these hints and model
     * (2) if not, compile the hints and cache the token
     * (3) set opts.COBALT_COMPILED_CONTEXT_DATA
     */
