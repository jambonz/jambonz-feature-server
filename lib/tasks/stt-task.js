...
class SttTask extends Task {
    ...
    if (
      ...
      try {
        ...
        if (this.canFallback) {
          ...
          if (ep) {
            await this._initFallback(ep);
          }
          if (ep2) {
            await this._initFallback(ep2);
          }
        } else {
          this.notifyError(
            {
  ...
  /**
   * Removes custom event listeners that were previously added for STT transcription.
   * This method is called during fallback scenarios to clean up listeners from the previous
   * STT vendor before switching to a fallback vendor, preventing memory leaks and
   * duplicate event handling.
   *
   * @param {Object} [ep] - Optional endpoint object. If provided, only removes listeners
   *                       associated with this specific endpoint. If not provided,
   *                       removes all custom event listeners from all endpoints.
   *
   * @example
   * // Remove all custom event listeners
   * this.removeCustomEventListeners();
   *
   * // Remove listeners only for a specific endpoint
   * this.removeCustomEventListeners(specificEndpoint);
   */
  ...
  /*
  * This method is called when the primary STT provider fails to transcribe the speech.
  * It sets the fallback vendor and language, and initializes the speech credentials.
  * It also removes the custom event listeners from the previous vendor.
  *
  * @param {Object} ep - The endpoint object. If sent, the custom event listeners will be removed
  *                      from the previous vendor for this endpoint.
  ...
  */
  async _initFallback(ep) {
    ...
    // ep is undefined from gather task
    if (ep) {
      ep.fallbackInitiated = true;
    }
