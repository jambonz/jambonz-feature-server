...
class SttTask extends Task {
  ...
  constructor(logger, data, parentTask) {
    ...
    if (this.data.recognizer) {
      ...
      // If there is no verb's recognizer, use application's recognizer for label
      // There is no access to call session, let use magic word for label.
      // this magic word will be replaced by task.exec who has access to call session.
      this.label = '{jambonz_label}';
      this.fallbackLabel = '{jambonz_label}';
    }
  ...
  async exec(cs, {ep, ep2}) {
    ...
    // label can be empty, should not assign application level label
    if ('{jambonz_label}' === this.fallbackLabel) {
