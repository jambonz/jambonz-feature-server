...
class SttTask extends Task {
  ...
  _onVendorError(cs, _ep, evt) {
    ...
    const {writeAlerts, AlertType} = cs.srf.locals;
    this.logger.info({evt}, `${this.name}:_on${this.vendor}_onVendorError`);
    writeAlerts({
      account_sid: cs.accountSid,
    ...
    this.notifyError({msg: 'ASR error', details:`Failed connecting to speech vendor ${this.vendor}: ${error}`});
  }
