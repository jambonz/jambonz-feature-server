...
class Dialogflow extends Task {
  ...
  async _onIntent(ep, cs, evt) {
    ...
    if (this.vendor && intent.fulfillmentText && intent.fulfillmentText.length > 0) {
      ...
      try {
        ...
        this.sayTask = makeTask(this.logger, {say}, this);
        const {span, ctx} = this.startChildSpan(`nested:${this.sayTask.summary}`);
        this.sayTask.span = span;
        this.sayTask.ctx = ctx;
        this.sayTask.on('playDone', (err) => {
          span.end();
          if (err) this.logger.error({err}, 'DialogFlow:_onIntent Error playing tts');
          this.logger.debug('DialogFlow: nested say task completed');
        });
        if (this.events.includes('start-play')) {
          this._performHook(cs, this.eventHook, {event: 'start-play', data: {text: say.text}});
        }
        this.playInProgress = true;
        await this.sayTask.exec(cs, {ep});
        if (this.events.includes('stop-play')) {
          this._performHook(cs, this.eventHook, {event: 'stop-play', data: {text: say.text}});
        }
  ...
  async _onAudioProvided(ep, cs, evt) {
    ...
    this.playTask = makeTask(this.logger, {play}, this);
    const {span, ctx} = this.startChildSpan(`nested:${this.playTask.summary}`);
    this.playTask.span = span;
    this.playTask.ctx = ctx;
    this.playTask.on('playDone', (err) => {
      span.end();
      if (err) this.logger.error({err}, 'DialogFlow:_onAudioProvided Error playing url');
      this.logger.debug('DialogFlow: nested play task completed');
    });
    ...
    this.playInProgress = true;
    await this.playTask.exec(cs, {ep});
    if (this.events.includes('stop-play')) {
