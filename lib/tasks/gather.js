...
class TaskGather extends SttTask {
  ...
  _onTranscription(cs, ep, evt, fsEvent) {
    ...
    if (evt.is_final) {
      ...
      this.stt_latency = `${sttLatency.stt_latency},`;
      this.stt_talkspurts.push([sttLatency.start_talking_time, sttLatency.stop_talking_time]);
  ...
  async _resolve(reason, evt) {
    ...
    if (this.needsStt) {
      const sttLatency = this.cs.calculateSttLatency();
      this.stt_latency = this.stt_latency.endsWith(',') ?
        this.stt_latency.slice(0, -1) : this.stt_latency;
      sttLatencyMetrics = {
        'stt.latency': this.stt_latency,
        'stt.talkspurts': JSON.stringify(this.stt_talkspurts),
