...
class TaskGather extends Task {
  constructor(logger, opts, parentTask) {
    ...
    if (this.data.recognizer) {
      ...
      /* let credentials be supplied in the recognizer object */
  ...
  async _initSpeech(cs, ep) {
    ...
    switch (this.vendor) {
      ...
      case 'deepgram':
        ...
        ep.addCustomEventListener(DeepgramTranscriptionEvents.Error, this._onDeepgramError.bind(this, cs, ep));
        ep.addCustomEventListener(DeepgramTranscriptionEvents.VadDetected, this._onVadDetected.bind(this, cs, ep));
        break;
  ...
  _onTranscription(cs, ep, evt, fsEvent) {
    ...
    if (evt.is_final) {
      if (evt.alternatives[0].transcript === '' && !this.callSession.callGone && !this.killed) {
        ...
        else if ('deepgram' === this.vendor) {
          this.logger.debug({evt}, 'TaskGather:_onTranscription - got empty transcript from deepgram, disregarding');
        }
        else {
  ...
  _onDeepgramError(_cs, _ep, evt) {
    this.logger.info({evt}, 'TaskGather:_onDeepgramError');
  }
