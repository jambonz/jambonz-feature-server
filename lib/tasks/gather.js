...
class TaskGather extends SttTask {
  ...
  _onTranscription(cs, ep, evt, fsEvent) {
    ...
    if (confidence && minConfidence && confidence < minConfidence) {
      this.logger.info({evt},
        'TaskGather:_onTranscription - rejecting transcript as confidence ' +
        `(${confidence}) < minConfidence (${minConfidence})`);
      evt.error = 'Rejecting transcript as confidence ' +
        `(${confidence}) < minConfidence (${minConfidence})`
      return this._resolve('stt-min-confidence-error', evt);
    }
  ...
  async _resolve(reason, evt) {
    ...
    try {
      ...
      } else if (reason.startsWith('stt-min-confidence-error')) {
        if (this.parentTask) this.parentTask.emit('stt-min-confidence-error', evt);
        else {
          this.emit('stt-min-confidence-error', evt);
          returnedVerbs = await this.performAction({reason: 'error', details: evt.error});
        }
      }
