...
class TaskGather extends SttTask {
  constructor(logger, opts, parentTask) {
    ...
    /* openai freeform prompt */
    this.openaiPrompt = null;
  }
  ...
  _startTranscribing(ep) {
    ...
    if (this.vendor === 'openai' && this.modelSupportsConversationTracking) {
      this.openaiPrompt = this.formatOpenAIPrompt(this.cs, {
        prompt: this.openaiPrompt,
        hintsTemplate: this.data.recognizer?.openaiOptions?.openaiOptions?.hintsTemplate,
        conversationHistoryTemplate: this.data.recognizer?.openaiOptions?.openaiOptions?.conversationHistoryTemplate,
        hints: this.data.recognizer?.hints,
      });
      this.logger.debug({openaiPrompt: this.openaiPrompt}, 'Gather:_startTranscribing - created an openai prompt');
    }
