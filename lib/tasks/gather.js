...
class TaskGather extends SttTask {
  ...
  async exec(cs, {ep}) {
    ...
    if (this.actionHookDelayAction) {
      if (cs.actionHookDelayProcessor) {
        // save so we can restore session-level properties when done
        this._storedActionHookDelayProperties = cs.actionHookDelayProcessor.properties;
      }
  ...
  async _resolve(reason, evt) {
    ...
    if (this.cs.actionHookDelayProcessor) {
      this.cs.actionHookDelayProcessor.stop();

      // if we had session level action hook delay properties, restore them
      if (this._storedActionHookDelayProperties) {
        this.cs.actionHookDelayProcessor.init(this._storedActionHookDelayProperties);
      }
    }
