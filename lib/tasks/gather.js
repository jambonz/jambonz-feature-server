...
class TaskGather extends SttTask {
  ...
  async exec(cs, {ep}) {
    ...
    if (this.data.minBargeinWordCount !== undefined) {
      this.minBargeinWordCount = this.data.minBargeinWordCount;
    } else if(cs?.bargeIn?.minBargeinWordCount !== undefined) {
      this.minBargeinWordCount = cs?.bargeIn?.minBargeinWordCount;
    } else {
      (cs.vad || this.data?.recognizer?.vad) ? this.minBargeinWordCount = 0 :
          this.minBargeinWordCount = 1;
    }
    this._vadEnabled = this.minBargeinWordCount === 0;
    ...
    this.vad = {
      ...(cs.vad || {}),
      ...(this.data?.recognizer?.vad || {})
    };
    if (this._vadEnabled && Object.keys(this.vad).length === 0) {
      this.vad = {
        "enable" : true,
        "mode": 2
      }
