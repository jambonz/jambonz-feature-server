...
class TaskGather extends SttTask {
  ...
  async exec(cs, {ep}) {
    ...
    // actionHook delay
    this._actionHookDelayEnabled = cs.actionHookDelayEnabled ||
      (this.actionHookDelayAction !== null && this.actionHookDelayAction !== undefined);
  ...
  _startActionHookNoResponseTimer() {
    ...
    this._actionHookNoResponseTimer = setTimeout(() => {
      ...
      if (verb.verb === 'say') {
        this._actionHookDelaySayTask(verb);
      } else if (verb.verb === 'play') {
        this._actionHookDelayPlayTask(verb);
      }
      this._actionHookDelayTryCount++;
      this._startActionHookNoResponseTimer();

    }, this._actionHookNoResponseTimer);
