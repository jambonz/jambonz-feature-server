...
class TaskGather extends SttTask {
  ...
  _startTranscribing(ep) {
    ...
    if (JAMBONES_TRACK_CONVERSATIONS && this.modelSupportsConversationTracking) {
      const turns = this.cs.getFormattedConversation(4);
      if (turns) {
        // eslint-disable-next-line max-len
        this.openaiPrompt = `${this.openaiPrompt || ''}\n\Transcribe what the user says.  The recent conversation to this point is as follows:\n${turns}\nuser:\n`;
        this.logger.debug({turns}, `Gather:_startTranscribing - prompt ${this.openaiPrompt}`);
      }
    }
    else {
      this.logger.debug('Gather:_startTranscribing - no conversation turns');
    }
