...
class TaskGather extends SttTask {
  ...
  _onTranscription(cs, ep, evt, fsEvent) {
    ...
    // There is a case where gather task is being resolved/killed,
    // but still receives transcription events. So ignore those events.
    if (this.resolved || this.killed) {
      this.logger.info('Gather:_onTranscription - task already resolved or killed, ignoring transcript');
      return;
    }
