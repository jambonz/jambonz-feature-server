...
class TaskGather extends Task {
  ...
  _onTranscription(cs, ep, evt, fsEvent) {
    ...
    const bufferedWords = this._bufferedTranscripts.reduce((count, e) => {
      return count + this.vendor === 'soniox' ?
        e.length :
        e.alternatives[0]?.transcript.split(' ').length;
    }, 0);
    ...
    if (evt.is_final) {
      ...
      if (this.isContinuousAsr) {
        ...
        this._bufferedTranscripts.push('soniox' === this.vendor ? evt.vendor.finalWords : evt);
        this._clearTimer();
        ...
        if (this.bargein && (words + bufferedWords) < this.minBargeinWordCount) {
          ...
          this._bufferedTranscripts.push('soniox' === evt.vendor ? evt.vendor.finalWords : evt);
          this._startTranscribing(ep);
          return;
        }
        else {
          if (this.vendor === 'soniox') {
            ...
            this._bufferedTranscripts.push(evt.vendor.finalWords);
            evt = this.compileSonioxTranscripts(this._bufferedTranscripts, 1, this.language);
            this._bufferedTranscripts = [];
          }
          this._resolve('speech', evt);
        }
