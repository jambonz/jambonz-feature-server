...
class TaskGather extends SttTask {
  ...
  _killAudio(cs) {
    ...
    if (this.sayTask && !this.sayTask.killed) {
      // before killing the sayTask, emit playDone so that any listeners get notified
      this.sayTask.emit('playDone');
      this.sayTask.removeAllListeners('playDone');
      this.sayTask.kill(cs);
      this.sayTask = null;
    }
    if (this.playTask && !this.playTask.killed) {
      // before killing the playTask, emit playDone so that any listeners get notified
      this.playTask.emit('playDone');
