    ...
    try {
      if (this.sayTask) {
        ...
        if (this.sayTask.isStreamingTts && !this.sayTask.closeOnStreamEmpty) {
          ...
          this.sayTask.on('playDone', ({error, task}) => {
            span.end();
            if (error) this.logger.error({error}, 'Gather:exec Error playing tts');
            if (task !== this.sayTask) {
              this.logger.warn('Gather:exec - received playDone from an unexpected sayTask');
              return;
            }
            process();
          });
        ...
        this.playTask.on('playDone', ({error, task}) => {
          span.end();
          if (error) this.logger.error({error}, 'Gather:exec Error playing url');
          if (task !== this.playTask) {
            this.logger.warn('Gather:exec - received playDone from an unexpected playTask');
            return;
          }
          process();
        });
