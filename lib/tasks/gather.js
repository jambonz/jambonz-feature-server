...
class TaskGather extends Task {
  constructor(logger, opts, parentTask) {
    ...
    if (this.data.recognizer) {
      ...
      if (recognizer.vendor === 'nuance') {
        ...
        const {apiKey, region, instanceId} = recognizer?.ibmOptions;
        if (apiKey) this.sttCredentials = {api_key: apiKey, region, instance_id: instanceId};
      }
  ...
  async exec(cs, {ep}) {
    ...
    else if (this.vendor == 'ibm' && this.sttCredentials.apikey) {
      /* get ibm access token */
      const {apikey, region} = this.sttCredentials;
      const {access_token, servedFromCache} = await getIbmAccessToken(apikey);
      this.logger.debug({apikey}, `Gather:exec - got ibm access token ${servedFromCache ? 'from cache' : ''}`);
      this.sttCredentials = {...this.sttCredentials, access_token, region};
    }
  ...
  async _initSpeech(cs, ep) {
    ...
    switch (this.vendor) {
      ...
      case 'ibm':
        ...
        ep.addCustomEventListener(IbmTranscriptionEvents.Connect, this._onDeepgramConnect.bind(this, cs, ep));
