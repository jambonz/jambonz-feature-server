  ...
  async _setSpeechHandlers(cs, ep) {
    ...
    switch (this.vendor) {
      ...
      case 'openai':
        ...
        /* append the last few turns of conversation to the prompt */
        if (JAMBONES_TRACK_CONVERSATIONS) {
          const turns = cs.getFormattedConversation(4);
          let prompt = opts.OPENAI_PROMPT || '';
          prompt += `\n\nHere is the recent history of the conversation:\n${turns}`;
          opts.OPENAI_PROMPT = prompt;
          this.logger.debug({prompt}, 'Gather:exec - setting OPENAI_PROMPT using conversation tracking');
        }
        break;
