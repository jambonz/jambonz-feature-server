...
class TaskGather extends SttTask {
  ...
  async _setSpeechHandlers(cs, ep) {
    ...
    /* if continuous asr on deepgram, use their utterance_end_ms property */
    if (this.isContinuousAsr && this.vendor === 'deepgram') this._doContinuousAsrWithDeepgram(this.asrTimeout);
  ...
  _startTranscribing(ep) {
    ...
    ep.startTranscription({
      vendor: this.vendor,
      locale: this.language,
      interim: this.interim && this.vendor !== 'deepgram',
      bugname: this.bugname,
  ...
  _onTranscription(cs, ep, evt, fsEvent) {
    ...
    if (this.vendor === 'deepgram' && evt.type === 'UtteranceEnd') {
      ...
      this.logger.debug('Gather:_onTranscription - got UtteranceEnd event from deepgram');
      evt = this.compileDeepgramTranscripts(this._bufferedTranscripts, 1, this.language);
      this._bufferedTranscripts = [];
      this._resolve(evt.alternatives?.length > 0 ? 'speech' : 'timeout', evt);
      return;
    }
    ...
    if (evt.is_final) {
      if (evt.alternatives[0].transcript === '' && !this.callSession.callGone && !this.killed) {
        ...
        if (finished === 'true' && ['microsoft', 'deepgram'].includes(this.vendor)) {
          ...
          this.logger.info({evt},
            'TaskGather:_onTranscription - got empty transcript from deepgram, returned buffered transcript');
        }
        else {
          this.logger.info({evt}, 'TaskGather:_onTranscription - got empty transcript, continue listening');
          return;
      ...
      if (this.isContinuousAsr) {
        ...
        if (this.vendor !== 'deepgram') this._startAsrTimer();
        ...
        if (this.bargein && (words + bufferedWords) < this.minBargeinWordCount) {
          ...
          if (this.vendor === 'soniox') {
            ...
            /*
              TODO: if we have enabled utterance_end_ms then we should not resolve here
              but instead wait for UtteranceEnd
            */
            /* compile transcripts into one */
