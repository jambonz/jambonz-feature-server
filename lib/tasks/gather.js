...
class TaskGather extends SttTask {
  ...
  async _onOpenAIPartialTranscript(cs, _ep, evt) {
    this.logger.debug({evt}, 'TaskGather:_onOpenAIPartialTranscript');
    if (!this.playComplete) {
      const words = evt.delta.split(' ').filter((w) => /A-Za-z0-0/.test(w));
      this.openaiPartials.push(words);
      this.logger.debug({words: this.openaiPartials, evt}, 'TaskGather:_onOpenAIPartialTranscript - words');
      if (words >= this.minBargeinWordCount) {
