...
class TaskGather extends Task {
  ...
  async exec(cs, {ep}) {
    ...
    else if (this.vendor == 'ibm' && this.sttCredentials.api_key) {
      /* get ibm access token */
      const {api_key, region} = this.sttCredentials;
      const {access_token, servedFromCache} = await getIbmAccessToken(api_key);
      this.logger.debug({api_key}, `Gather:exec - got ibm access token ${servedFromCache ? 'from cache' : ''}`);
