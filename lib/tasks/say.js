...
class TaskSay extends Task {
  ...
  async _synthesizeWithSpecificVendor(cs, ep, {vendor, language, voice, label}) {
    ...
    if (vendor === 'microsoft' && this.options.deploymentId) {
      ...
      credentials.api_key = this.options.apiKey || credentials.api_key;
      ...
      credentials.optimize_streaming_latency = this.options.optimize_streaming_latency || credentials.optimize_streaming_latency;
