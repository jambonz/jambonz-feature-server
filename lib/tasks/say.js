    ...
    while (!this.killed && (this.loop === 'forever' || this.loop--) && ep?.connected) {
      ...
      while (!this.killed && segment < filepath.length) {
        ...
        if (cs.isInConference) {
          ...
          // Start a timeout timer as a last resort to prevent hanging forever
          // https://jambonz.freshdesk.com/a/tickets/1629
          // Azure tts streaming for whitespace string will not trigger playback-start to
          // register playback-stop listener that will cause hanging forever here
          const timeoutTimer = setTimeout(() => {
            if (this._playReject) {
              this.logger.warn('Say:exec playback timeout - no stop event received within timeout period');
              this._playReject(new Error('Playback finished without stop event'));
            }
          }, 500);
