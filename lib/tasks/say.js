...
const { makeSynthKey } = require('@jambonz/speech-utils/lib/utils');
      ...
      while (!this.killed && segment < filepath.length) {
        if (cs.isInConference) {
          ...
          const onPlaybackStop = (evt) => {
            try {
              // generate a key for the text segment
              // we will check whether the key matches the key in the playback-start event
              let key;
              if (evt.variable_tts_playback_identifier) {
                key = makeSynthKey({text: this.text[segment]});
              }
              this.logger.info({evt, key},
                `Say got playback-stop ${evt.variable_tts_playback_identifier ?
                  evt.variable_tts_playback_identifier : ''}`);

              if (evt.variable_tts_playback_identifier) {
                // check if the key matches the key in the playback-start event
                if (key !== playbackId) {
                  this.logger.info({currentPlaybackId: playbackId, stopPlaybackId: evt.variable_tts_playback_identifier,
                    playbackId, key},
                  'Say got playback-stop text mismatch between playback-start and playback-stop');
          ...
          ep.once('playback-start', (evt) => {
            try {
              playbackId = evt.variable_tts_playback_identifier;
              this.logger.info(evt,
                `Say got playback-start ${evt.variable_tts_playback_identifier ?
                  evt.variable_tts_playback_identifier : ''}`);
              if (this.otelSpan) {
          ...
          const key = makeSynthKey({text: this.text[segment]});
          const r = await ep.play(filepath[segment], null, key);
