...
const isUnmatched = (filename, playbackId, evt) => {
  return (
    (!!playbackId && !!evt.variable_tts_playback_id && evt.variable_tts_playback_id !== playbackId) // (1)
    || (!!playbackId && !evt.variable_tts_playback_id) // (2)
    || (!playbackId && !!evt.variable_tts_playback_id) // (3)
    || (!!filename && !!evt.variable_tts_cache_filename && evt.variable_tts_cache_filename !== filename) // (4)
  );
};
        ...
        if (cs.isInConference) {
          ...
          const onPlaybackStop = (evt) => {
            try {
              ...
              const unmatchedResponse = isUnmatched(filename, playbackId, evt);
