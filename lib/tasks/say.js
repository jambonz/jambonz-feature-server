      ...
      const generateAudio = async(text) => {
        ...
        try {
          ...
          if (!filePath.startsWith('say:')) {
            this.logger.debug({text}, `Say: file ${filePath}, served from cache ${servedFromCache}`);
    ...
    while (!this.killed && (this.loop === 'forever' || this.loop--) && ep?.connected) {
      ...
      while (!this.killed && segment < filepath.length) {
        if (cs.isInConference) {
          ...
          ep.on('playback-stop', (evt) => {
            const {variable_tts_cache_filename} = evt;
            if (!tts_cache_filename || evt.tts_cache_filename !== tts_cache_filename) {
              this.logger.info(`Say: discarding playback-stop from prev say${variable_tts_cache_filename}`);
              return;
            }
