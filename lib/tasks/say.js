...
class TaskSay extends Task {
  ...
  async exec(cs, {ep}) {
    ...
    if (cs.isFallbackTts) {
      this.vendor = this.fallbackLabel;
      this.language = this.fallbackLanguage;
      this.voice = this.fallbackVoice;
    ...
    try {
      ...
      if (fallbackVendor && this.isHandledByPrimaryProvider && !cs.isFallbackTts) {
        this.notifyError({ msg: 'TTS error', details:`Invalid vendor ${this.vendor}`, failover: 'in progress'});
        this.isHandledByPrimaryProvider = false;
        cs.isFallbackTts = true;
        ...
        this.notifyError({ msg: 'TTS error', details:`Invalid vendor ${this.vendor}`, failover: 'not available'});
        throw error;
      }
    }
