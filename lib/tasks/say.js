  ...
  async exec(cs, {ep}) {
    ...
    const startFallback = async(error) => {
      if (fallbackVendor && this.isHandledByPrimaryProvider && !cs.hasFallbackTts) {
        ...
        this._filepath = await this._synthesizeWithSpecificVendor(cs, ep,
          {
            vendor: fallbackVendor,
            language: fallbackLanguage,
            voice: fallbackVoice,
    ...
    try {
      this._filepath = await this._synthesizeWithSpecificVendor(cs, ep, {vendor, language, voice, label});
    } catch (error) {
    ...
    while (!this.killed && (this.loop === 'forever' || this.loop--) && ep?.connected) {
      let segment = 0;
      while (!this.killed && segment < this._filepath.length) {
        if (cs.isInConference) {
          const {memberId, confName, confUuid} = cs;
          await this.playToConfMember(ep, memberId, confName, confUuid, this._filepath[segment]);
        }
        else {
          if (this._filepath[segment].startsWith('say:{')) {
            const arr = /^say:\{.*\}\s*(.*)$/.exec(this._filepath[segment]);
            if (arr) this.logger.debug(`Say:exec sending streaming tts request: ${arr[1].substring(0, 64)}..`);
          }
          else this.logger.debug(`Say:exec sending  ${this._filepath[segment].substring(0, 64)}`);
          ...
          await ep.play(this._filepath[segment]);
          try {
            // wait for playback-stop event received to confirm if the playback is successful
          ...
          this.logger.debug(`Say:exec completed play file ${this._filepath[segment]}`);
        }
        segment++;
      }
    }
