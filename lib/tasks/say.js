...
const breakLengthyTextIfNeeded = (/*logger, */ text) => {
  ...
  while (startPos + charPos < text.length) {
    if (tag) {
      // we are inside a tag, proceed to the end
      if (0 === text.indexOf(`</${tag}>`, startPos + charPos)) {
        //logger.debug(`breakLengthyTextIfNeeded: exit tag ${tag}`);
        charPos += tag.length + 3;
        tag = null;
      }
      else charPos++;
      continue;
    }
    if (isSSML && 0 === text.indexOf('<', startPos + charPos)) {
      // we are at the beginning of a tag, proceed to the end
      const endPos = text.indexOf('>', startPos + charPos);
      if (endPos > 0) {
        tag = text.substring(startPos + charPos + 1, endPos);
        //logger.debug(`breakLengthyTextIfNeeded: enter tag ${tag}`);
        charPos += tag.length + 2;
        continue;
      }
    }
    ...
    let chunkIt = false;
    if (text[startPos + charPos] === '.' && text[startPos + charPos + 1] === ' ') chunkIt = true;
    else if (charPos > chunkSize + 100 && text[startPos + charPos] === ' ') chunkIt = true;
    if (chunkIt) {
      charPos++;
