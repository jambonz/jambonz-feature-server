...
class TaskSay extends TtsTask {
  ...
  notifyTtsStreamIsEmpty() {
    if (this.isStreamingTts && this.closeOnStreamEmpty) {
      ...
      this.cs.requestor?.request('tts:streaming-event', '/streaming-event', {event_type: 'stream_empty'})
        .catch((err) => this.logger.info({err}, 'TaskSay:notifyTtsStreamIsEmpty - Error sending'));
      this.notifyTaskDone();
    }
  }
}
