      ...
      while (!this.killed && segment < filepath.length) {
        if (cs.isInConference) {
          ...
          this.ep.once('playback-stop', (evt) => {
            this.logger.info({evt}, 'got playback-stop');
            if (evt.variable_tts_cache_filename) {
              //TODO: save the cached file contents to redis and have session track the file to delete later
              // remove file when session ends
              //TODO: this unlink of the file is failing due to permissions errors
