...
class TaskSay extends Task {
      ...
      while (!this.killed && segment < filepath.length) {
        if (cs.isInConference) {
          ...
          this.ep.once('playback-start', (evt) => {
            if (this.otelSpan) {
              this.logger.info({evt}, 'got playback-start');
  ...
  _addStreamingTtsAttributes(span, evt) {
    const attrs = {
      'tts.cached': false,
      ...(evt.elevenlabs_reported_latency_ms &&
        {'elevenlabs.latency_ms': evt.elevenlabs_reported_latency_ms}),
      ...(evt.elevenlabs_request_id &&
        {'elevenlabs.request_id': evt.elevenlabs_request_id}),
      ...(evt.elevenlabs_history_item_id &&
        {'elevenlabs.history_id': evt.elevenlabs_history_item_id}),
      ...(evt.elevenlabs_name_lookup_time_ms &&
        {'elevenlabs.name_lookup_ms': evt.elevenlabs_name_lookup_time_ms}),
      ...(evt.elevenlabs_connect_time_ms &&
        {'elevenlabs.connect_ms': evt.elevenlabs_connect_time_ms}),
      ...(evt.elevenlabs_final_response_time_ms &&
        {'elevenlabs.final_response_ms': evt.elevenlabs_final_response_time_ms}),
      ...(evt.tts_time_to_first_byte_ms &&
        {tts_time_to_first_byte_ms: evt.tts_time_to_first_byte_ms}),
      ...(evt.elevenlabs_model_id &&
        {'elevenlabs.model_id': evt.elevenlabs_model_id}),
      ...(evt.elevenlabs_optimize_streaming_latency &&
        {'elevenlabs.optimize': evt.elevenlabs_optimize_streaming_latency}),
    };
