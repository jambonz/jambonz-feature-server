        ...
        if (cs.isInConference) {
          ...
          const onPlaybackStop = (evt) => {
            try {
              ...
              let key;
              if (evt.variable_tts_playback_id) {
              ...
              if (evt.variable_tts_playback_id && cs.idTextMap.has(evt.variable_tts_playback_id)) {
                // check if the key matches the key in the playback-start event
                const playbackStartKey = cs.idTextMap.get(evt.variable_tts_playback_id);
                if (key === playbackStartKey) {
                  cs.idTextMap.delete(evt.variable_tts_playback_id);
                } else {
                  this.logger.info({currentPlaybackId: playbackId, stopPPlaybackId: evt.variable_tts_playback_id,
                    playbackStartKey, key},
          ...
          ep.once('playback-start', (evt) => {
            try {
              ...
              let key;
              if (playbackId) {
                // create a key for the text, so that we can match the key in the playback-stop event
                key = makeSynthKey({text: this.text[segment]});
                cs.idTextMap.set(playbackId, key);
              }
              this.logger.info({...evt, key},
