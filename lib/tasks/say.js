    ...
    while (!this.killed && (this.loop === 'forever' || this.loop--) && ep?.connected) {
      ...
      while (!this.killed && segment < filepath.length) {
        if (cs.isInConference) {
          ...
          const str = filepath[segment];
          const textSent = str.startsWith('say:{') && str.includes('}') ?
          ...
          ep.once('playback-stop', (evt) => {
            ...
            const {variable_tts_text, ...evtMinusText} = evt;
            this.logger.debug({evtMinusText}, 'Say got playback-stop');
            const unmatchedResponse = !!evt.variable_tts_text && textSent !== evt.variable_tts_text;
            if (unmatchedResponse) {
              this.logger.info('Say:exec discarding playback-stop - text sent does not match text received');
              return;
            }
