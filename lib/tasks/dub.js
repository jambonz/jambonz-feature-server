...
class TaskDub extends TtsTask {
  ...
  async _addTrack(cs, ep) {
    ...
    /* if gain was specified, set it */
    if (this.gain) {
      this.logger.info(`setting gain on track: ${this.track} to ${this.gain}`);
      await ep.api('uuid_dub', [ep.uuid, 'setGain', this.track, this.gain]);
    }
