...
class TtsTask extends Task {
  ...
  async _synthesizeWithSpecificVendor(cs, ep, {vendor, language, voice, label, preCache = false}) {
    ...
    if (vendor === 'microsoft' && this.options.deploymentId) {
      ...
      credentials.model_id = this.options.model_id || credentials.model_id;
    } else if (vendor === 'google' && credentials.credentials.use_gemini_tts) {
      credentials.model_id = this.options.model_id || credentials.credentials.model_id;
      credentials.use_gemini_tts = credentials.credentials.use_gemini_tts;
    }
