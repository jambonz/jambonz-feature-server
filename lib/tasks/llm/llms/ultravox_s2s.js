...
class TaskLlmUltravox_S2S extends Task {
  ...
  async createCall() {
    // inject mcpTools into the payload, mcpServer will override the default tools
    // if mcpServer is not set, then llmOptions.selectedTools will be used.
    if (this.data.mcpServer) {
      const tools = await this.parent.listMcpTools();
      if (tools && tools.length > 0) {
        this.data.llmOptions.selectedTools = tools.map((tool) => {
          return {
            temporaryTool: {
              modelToolName: tool.name,
              description: tool.description,
              dynamicParameters: this.transformSchemaToParameters(tool.input_schema),
  ...
  async _onServerEvent(_ep, evt) {
    ...
    /* tool calls */
      ...
      if (this.data.mcpServer) {
        ...
        try {
          const res = await this.parent.callMcpTool(name, args);
