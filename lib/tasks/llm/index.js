...
class TaskLlm extends Task {
  ...
  _processLlmUpdate(data, callSid) {
    const {vendor, model} = this.llm;
    this.logger.info({data, callSid}, `TaskLlm:_processLlmUpdate: LLM ${vendor}:${model} does not support llm:update`);
  }

  _installLlmDefaults() {
    if (typeof this.llm.processLlmUpdate !== 'function') {
      this.llm.processLlmUpdate = this._processLlmUpdate.bind(this);
    }
  }
