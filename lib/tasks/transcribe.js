...
class TaskTranscribe extends SttTask {
    ...
    switch (this.vendor) {
      ...
      case 'deepgram':
        ...
        ep.addCustomEventListener(DeepgramTranscriptionEvents.Connect,
          this._onVendorConnect.bind(this, cs, ep, this.vendor));
        ep.addCustomEventListener(DeepgramTranscriptionEvents.ConnectFailure,
          this._onVendorConnectFailure.bind(this, cs, ep, channel, this.vendor));
  ...
  _onVendorConnect(_cs, _ep, vendor) {
    this.logger.debug(`TaskTranscribe:_on${vendor}Connect`);
  }

  _onVendorConnectFailure(cs, _ep, channel, vendor, evt) {
    const {reason} = evt;
    const {writeAlerts, AlertType} = cs.srf.locals;
    this.logger.info({evt}, `TaskTranscribe:_on${vendor}ConnectFailure`);
    writeAlerts({
      account_sid: cs.accountSid,
      alert_type: AlertType.STT_FAILURE,
      message: `Failed connecting to ${vendor} speech recognizer: ${reason}`,
      vendor,
    }).catch((err) => this.logger.info({err}, `Error generating alert for ${vendor} connection failure`));
    this.notifyError(`Failed connecting to speech vendor ${vendor}: ${reason}`);
