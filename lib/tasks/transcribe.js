...
const { JAMBONES_TRANSCRIBE_EP_DESTROY_DELAY_MS } = require('../config');
...
class TaskTranscribe extends SttTask {
  ...
  async _stopTranscription() {
    ...
    const stopTranscriptionArgs = {
      vendor: this.vendor,
      bugname: this.bugname,
    };
    if (JAMBONES_TRANSCRIBE_EP_DESTROY_DELAY_MS && this.paused) {
      // in case of pause, we want to stop transcription immediately
      stopTranscriptionArgs.immediate = true;
    }
    if (this.transcribing1 && this.ep?.connected) {
      stopTranscription = true;
      this.ep.stopTranscription(stopTranscriptionArgs)
        .catch((err) => this.logger.info(err, 'Error TaskTranscribe:kill'));
    }
    if (this.transcribing2 && this.ep2?.connected) {
      stopTranscription = true;
      this.ep2.stopTranscription(stopTranscriptionArgs)
        .catch((err) => this.logger.info(err, 'Error TaskTranscribe:kill'));
    }
  ...
  async _startFallback(cs, _ep, evt) {
    if (this.canFallback) {
      ...
      if (JAMBONES_TRANSCRIBE_EP_DESTROY_DELAY_MS) {
        stopTranscriptionArgs.immediate = true;
      }
      _ep.stopTranscription(stopTranscriptionArgs)
