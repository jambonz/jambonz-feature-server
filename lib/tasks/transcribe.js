...
class TaskTranscribe extends SttTask {
  ...
  async _startFallback(cs, _ep, evt) {
    // _ep.fallbackInitiated is set to true when _initFallback is called
    // when fallbackInitiated is true, it means the fallback has already been initiated
    // for the same endpoint, so we don't need to initiate it again
    if (this.canFallback && !_ep.fallbackInitiated) {
  ...
  async _onJambonzError(cs, _ep, evt) {
    ...
    // As we are now removing the custom event listeners of the endpoint,
    // if ep2 is available, it will also throw the error and control will come here,
    // so we need to reset the flags for fallback to work for ep2
    this.isHandledByPrimaryProvider = true;
    this.cs.hasFallbackAsr = false;
