...
class TaskTranscribe extends Task {
  ...
  async _resolve(channel) {
    const evt = this.compileTranscripts();
    this._bufferedTranscripts = [];
  ...
  _startAsrTimer(channel) {
    ...
    this._asrTimers[channel] = setTimeout(() => {
  ...
  _clearAsrTimer(channel) {
    const asrTimer = this._asrTimers[channel];
    if (asrTimer) clearTimeout(asrTimer);
    this._asrTimers[channel] = null;
  }
}
