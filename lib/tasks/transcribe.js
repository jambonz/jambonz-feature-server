...
class TaskTranscribe extends SttTask {
  constructor(logger, opts, parentTask) {
    ...
    // bugname prefix is for knowing background/foreground task is runnign
  ...
  async _stopTranscription() {
    ...
    if (this.ep?.connected) {
      ...
      this.ep.stopTranscription({
        vendor: this.vendor,
        bugname: this.bugnames.get(this.ep)
      })
        .catch((err) => this.logger.info(err, 'Error TaskTranscribe:kill'));
    }
    if (this.separateRecognitionPerChannel && this.ep2 && this.ep2.connected) {
      stopTranscription = true;
      this.ep2.stopTranscription({vendor: this.vendor,  bugname: this.bugnames.get(this.ep2)})
        .catch((err) => this.logger.info(err, 'Error TaskTranscribe:kill'));
    }
  ...
  async _setSpeechHandlers(cs, ep, channel) {
    ...
    this.bugnames.set(ep, `${bugname}_${Date.now()}`);
    await ep.set(opts)
  ...
  async _transcribe(ep) {
    this.logger.debug(
      `TaskTranscribe:_transcribe - starting transcription vendor ${this.vendor} bugname ${this.bugnames.get(ep)}`);
    await ep.startTranscription({
      vendor: this.vendor,
      interim: this.interim ? true : false,
      locale: this.language,
      channels: /*this.separateRecognitionPerChannel ? 2 : */ 1,
      bugname: this.bugnames.get(ep),
  ...
  async _onTranscription(cs, ep, channel, evt, fsEvent) {
    ...
    if (bugname && this.bugnames.get(ep) !== bugname) return;
