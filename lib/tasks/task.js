...
class Task extends Emitter {
  ...
  async performAction(results, expectResponse = true) {
    if (this.actionHook) {
      ...
      const childSpan = this.span.startChildSpan(type, {
        'hook.url': this.actionHook,
        'http.body': JSON.stringify(params)
      });
      const b3 = childSpan.getTracingPropagation('b3');
      const httpHeaders = b3 && {b3};
      try {
        const json = await this.cs.requestor.request(type, this.actionHook, params, httpHeaders);
        childSpan.setAttributes({'http.statusCode': 200});
        childSpan.end();
        ...
        childSpan.setAttributes({'http.statusCode': err.statusCode});
  ...
  async performHook(cs, hook, results) {
    ...
    const childSpan = this.span.startChildSpan('verb:hook', {
      'hook.url': hook,
      'http.body': JSON.stringify(params)
    });
    const b3 = childSpan.getTracingPropagation('b3');
