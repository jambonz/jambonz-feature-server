...
class TaskEnqueue extends Task {
  ...
  async _addToQueue(cs, dlg) {
    const {addToSortedSet} = cs.srf.locals.dbHelpers;
    const url = getUrl(cs);
    this.waitStartTime = Date.now();
    this.logger.debug({queue: this.queueName, url}, 'pushing url onto queue');
    if (this.priority < 0) {
      this.logger.warn(`priority ${this.priority} is invalid, need to be non-negative integer, 999 will be used for priority`);
    }
    const members = await addToSortedSet(this.queueName, url, this.priority);
