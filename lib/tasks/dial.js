  ...
  get summary() {
    if (this.target.length === 1) {
      ...
      switch (target.type) {
        case 'phone':
        case 'teams':
          return `${this.name}{type=${this.target.type},number=${target.number}}`;
        case 'user':
          return `${this.name}{type=${this.target.type},name=${target.name}}`;
        case 'sip':
          return `${this.name}{type=${this.target.type},sipUri=${target.sipUri}}`;
        default:
          return `${this.name}`;
      }
    }
