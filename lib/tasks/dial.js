        ...
        if (json && Array.isArray(json)) {
          try {
            ...
            if (tasks && tasks.length > 0) {
              ...
              this.logger.info(`Dial:handleRefer got REFER on ${legs[0]} leg, setting new app  on ${legs[1]} leg`);
              if (isChild) this.redirect(cs, tasks);
              else {
                this.logger.info({tasks}, 'Dial:handleRefer - new application for for child leg');
                ...
                cs.sessionTracker.add(adultingSession.callSid, adultingSession);
              }
            }
          } catch (err) {
