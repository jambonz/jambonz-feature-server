...
class TaskDial extends Task {
  ...
  /**
   * Check if two SDP endpoints are using the same primary audio codec
   * @param {object} ep1 - First endpoint object
   * @param {object} ep2 - Second endpoint object
   * Sets this.mediaNotSame to false if same codec (no transcoding needed), true otherwise
   */
  _checkTranscoding(ep1, ep2) {
    try {
      const sdp1 = ep1?.remote?.sdp;
      const sdp2 = ep2?.remote?.sdp;
      ...
      const parsed1 = sdpTransform.parse(sdp1);
      const parsed2 = sdpTransform.parse(sdp2);

      // Get audio media sections
      const audio1 = parsed1.media?.find((m) => m.type === 'audio');
      const audio2 = parsed2.media?.find((m) => m.type === 'audio');
      ...
      // Get first (primary) codec from each SDP
      const codec1 = audio1.rtp?.[0]?.codec;
      const codec2 = audio2.rtp?.[0]?.codec;
      ...
      this.mediaNotSame = codec1.toLowerCase() !== codec2.toLowerCase();
      this.logger.debug({codec1, codec2, mediaNotSame: this.mediaNotSame}, '_checkTranscoding result');
    } catch (err) {
      this.logger.error({err}, '_checkTranscoding error');
      this.mediaNotSame = true;
    }
  }
 ...
 media already released, reconnect media server`);
  ...
  async _selectSingleDial(cs, sd) {
    ...
    this._checkTranscoding(this.epOther, sd.ep);
    /* if we can release the media back to the SBC, do so now */
