...
class TaskDial extends Task {
  ...
  async _connectSingleDial(cs, sd) {
    ...
    if (cs.onHoldMusic) {
      sd.ep.set({hold_music: `shout://${this.cs.onHoldMusic.replace(/^https?:\/\//, '')}`});
    }
    ...
    try {
      ...
      if (this.cs.onHoldMusic) {
        if (isOnhold(req.body) && !this.epOther && !this.ep) {
          ...
          // reconnect A Leg and Update B leg
          await Promise.all([this.cs.reAnchorMedia(), this.sd.reAnchorMediaForResponse(res)]);
          this.epOther = this.cs.ep;
          // newly created endpoint, let reset the music onhold
          this.ep.set({hold_music: `shout://${this.cs.onHoldMusic.replace(/^https?:\/\//, '')}`});
          this.epOther.set({hold_music: `shout://${this.cs.onHoldMusic.replace(/^https?:\/\//, '')}`});
          // Connect 2 leg
          this.epOther.bridge(this.ep);
          this.logger.info('Dial:_onReinvite - successfully connect media from freewitch');
          isHandled = true;
        } else if (!isOnhold(req.body) && this.epOther && this.ep && this.canReleaseMedia) {
          // Offhold, time to release media
          const aLegSdp = this.cs.ep.remote.sdp;
          const bLegSdp = this.sd.dlg.remote.sdp;
          // release media A and B leg
          await Promise.all([
            this.sd.releaseMediaToSBCForResponse(res, aLegSdp, this.cs.ep.local.sdp),
            this.cs.releaseMediaToSBC(bLegSdp)]
          );
          this.epOther = null;
          this.logger.info('Dial:_onReinvite - successfully released media from freewitch');
          isHandled = true;
        }
      }
      if (!isHandled) {
  ...
  async handleReinviteAfterMediaReleased(req, res) {
    ...
    if (isOnhold(req.body) && !this.epOther && !this.ep) {
      // Onhold but media is already released
      await Promise.all([this.sd.reAnchorMedia(), this.cs.reAnchorMediaForResponse(res)]);
      this.epOther = this.cs.ep;
      // newly created endpoint, let reset the music onhold
      this.ep.set({hold_music: `shout://${this.cs.onHoldMusic.replace(/^https?:\/\//, '')}`});
      this.epOther.set({hold_music: `shout://${this.cs.onHoldMusic.replace(/^https?:\/\//, '')}`});
      // Connect 2 leg
      this.epOther.bridge(this.ep);
      this.logger.info('Dial:handleReinviteAfterMediaReleased - successfully connect media to freewitch');
      isHandled = true;
    } else if (!isOnhold(req.body) && this.epOther && this.ep && this.canReleaseMedia) {
      // Offhold, time to release media
      const aLegSdp = this.cs.ep.remote.sdp;
      const bLegSdp = this.sd.dlg.remote.sdp;
      await Promise.all([this.cs.releaseMediaToSBCForResponse(res, bLegSdp), this.sd.releaseMediaToSBC(aLegSdp)]);
      this.epOther = null;
      this.logger.info('Dial:handleReinviteAfterMediaReleased - successfully released media from freewitch');
      isHandled = true;
    }
