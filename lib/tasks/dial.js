...
class TaskDial extends Task {
 ...
 media already released, reconnect media server`);
  ...
  async _selectSingleDial(cs, sd) {
    ...
    const codecA = getLeadingCodec(this.ep.local.sdp);
    const codecB = getLeadingCodec(this.ep.remote.sdp);
    this.mediaNotSame = !(codecA === codecB);
    this.logger.info({A: codecA, B: codecB, Transcoding: this.mediaNotSame},
      'Dial:_selectSingleDial - is call being transcoded');
    /* if we can release the media back to the SBC, do so now */
