...
class TaskDial extends Task {
 ...
 media already released, reconnect media server`);
  ...
  async _selectSingleDial(cs, sd) {
    ...
    if (!this.canReleaseMedia) {
      this.sd.ep.once('destroy', this._onSingleDialEndpointDestroy.bind(this));
    }
  }

  _onSingleDialEndpointDestroy() {
    if (this.sd && this.bridged) {
      //sd ep is dead while call is still connect, hangup sd
      this.kill(this.cs, 'media_timeout');
    }
  }
