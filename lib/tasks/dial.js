...
class TaskDial extends Task {
  ...
  _onInfo(cs, dlg, req, res) {
    ...
    if (req.get('Content-Type') !== 'application/dtmf-relay') {
      return this._onIndilogRequest(cs, req, res);
    }
  ...
  _initSipIndialogRequestListener(cs, dlg) {
    dlg.on('message', this._onIndilogRequest.bind(this, cs));
  }
  ...
  async _onIndilogRequest(cs, req, res) {
    cs._onIndilogRequest(req, res);
  }
  ...
  async _selectSingleDial(cs, sd) {
    ...
    if (cs.sipIndialogActionHook) this._initSipIndialogRequestListener(cs, this.dlg);
