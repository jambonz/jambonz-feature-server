...
class TaskDial extends Task {
    ...
    try {
      ...
      if (this.cs.onHoldMusic) {
        if (isOnhold(req.body) && !this.epOther && !this.ep) {
          ...
          await Promise.all([this.cs.reAnchorMedia(), this.sd.reAnchorMediaForReInvite(res)]);
          ...
          await Promise.all([
            this.sd.releaseMediaToSBCForReInvite(res, aLegSdp, this.cs.ep.local.sdp),
  ...
  async handleReinviteAfterMediaReleased(req, res) {
    ...
    if (isOnhold(req.body) && !this.epOther && !this.ep) {
      // Onhold but media is already released
      await Promise.all([this.sd.reAnchorMedia(), this.cs.reAnchorMediaForReInvite(res)]);
      this.epOther = this.cs.ep;
      // Connect 2 leg
      this.epOther.bridge(this.ep);
      ...
      const aLegSdp = this.cs.ep.remote.sdp;
      const bLegSdp = this.sd.dlg.remote.sdp;
      await Promise.all([this.cs.releaseMediaToSBCForReInvite(res, bLegSdp), this.sd.releaseMediaToSBC(aLegSdp)]);
      this.epOther = null;
    ...
    if (this.data.onHoldHook) {
      ...
      do {
        try {
          const b3 = this.getTracingPropagation();
          const httpHeaders = b3 && {b3};
          const json = await this.cs.application.requestor.request('verb:hook', this.data.onHoldHook, this.cs.callInfo.toJSON(), httpHeaders);
      ...
      } while (tasks && tasks.length > 0 && !this.killed && this.isHold && (this.isIncomingLegHold || this.isOutgoingLegHold));
