...
class Conference extends Task {
  ...
  _onAddMember(logger, cs, evt) {
    if (this.speakOnlyTo) {
      logger.debug(`Conference:_onAddMember - member added to ${this.confName}, update coaching mode`);
      //this.setCoachMode(this.speakOnlyTo).catch(() => {});
    }
    else logger.debug(`Conference:_onAddMember - member added to conference ${this.confName}`);
  }
  ...
  _onTag(logger, cs, evt) {
    ...
    if (memberId !== this.memberId && this.speakOnlyTo) {
      logger.debug({evt}, `Conference:_onTag - member ${memberId} set tag ${tag}; updating coach mode accordingly`);
      this.setCoachMode(this.speakOnlyTo).catch(() => {});
    }
  }
  ...
  async setCoachMode(speakOnlyTo) {
    ...
    try {
      ...
      if (members.length === 0) {
        ...
        this.logger.info({members}, 'Conference:_setCoachMode: my audio will NOT be sent to these participants');
        await this.ep.api('conference', [this.confName, 'relate', this.memberId, members.join(','), 'nospeak']);
