...
class Conference extends Task {
  ...
  async _joinConference(cs, dlg, startConf) {
    ...
    try {
      ...
      if (this.data.memberTag) {
        this.ep.api('conference', [this.confName, 'tag', memberId, this.data.memberTag])
          .catch((err) => this.logger.error(err, `Error setting tag for ${memberId} to ${this.data.memberTag}`));
      }
      ...
      if (this.data.speakOnlyTo) {
        try {
          const response = await this.ep.api('conference', [this.confName, 'gettag', this.data.speakOnlyTo, 'nomatch']);
          const parts = response.split(' ');
          this.logger.info({response}, `Conference:_joinConference: gettag ${this.data.speakOnlyTo}`);
          await this.ep.api('conference', [this.confName, 'relate', response, 'nospeak']);
          this.logger.info(`Conference:_joinConference: my audio will only be heard by ${parts.length} members`);
        } catch (err) {
          this.logger.error(err, `Error getting tag ${this.data.speakOnlyTo}`);
        }
      }
    } catch (err) {
  ...
  _onAddMember(logger, cs, evt) {
    this.logger.debug({evt}, `Conference:_onAddMember - member added to conference ${this.confName}`);
  }
