...
class Rasa extends Task {
  ...
  async exec(cs, {ep}) {
    ...
    try {
      ...
      this.gatherTask.span = this.span.startChildSpan(`nested:${this.gatherTask.summary}`);
      this.gatherTask.exec(cs, {ep})
        .then(() => this.gatherTask.span.end())
        .catch((err) => {
          this.logger.info({err}, 'Rasa gather task returned error');
          this.gatherTask.span.endWithError('Rasa gather task returned error');
        });
