...
class Lex extends Task {
  ...
  _onIntent(ep, cs, evt) {
    ...
    if (this.events.includes('intent')) {
      this.performHook(cs, this.eventHook, {event: 'intent', data: evt});
    }
  }
  ...
  _onTranscription(ep, cs, evt) {
    ...
    if (this.events.includes('transcription')) {
      this.performHook(cs, this.eventHook, {event: 'transcription', data: evt});
    }
  }
  ...
  async _onTextResponse(ep, cs, evt) {
    ...
    if (this.events.includes('response-text')) {
      this.performHook(cs, this.eventHook, {event: 'response-text', data: evt});
    }
    if (this.vendor && Array.isArray(messages) && messages.length) {
      ...
      if (['PlainText', 'SSML'].includes(type) && msg) {
        try {
          const say = {
            text: msg,
            synthesizer: this.data.tts
          };
          const sayTask = makeTask(this.logger, {say}, this);
          const {span, ctx} = this.startChildSpan(`nested:${sayTask.summary}`);
          sayTask.span = span;
          sayTask.ctx = ctx;
          sayTask.on('playDone', (err) => {
            span.end();
            if (err) this.logger.error({err}, 'Lex:_onTextResponse Error playing tts');
            this.logger.debug('Lex: nested say task completed');
          });

          if (this.events.includes('start-play')) {
            this.performHook(cs, this.eventHook, {event: 'start-play', data: {text: msg}});
          }
          await sayTask.exec(cs, {ep});
          if (this.events.includes('stop-play')) {
            this.performHook(cs, this.eventHook, {event: 'stop-play', data: {text: msg}});
          }
  ...
  _onPlaybackInterruption(ep, cs, evt) {
    ...
    if (this.bargein) {
      if (this.events.includes('play-interrupted')) {
        this.performHook(cs, this.eventHook, {event: 'play-interrupted', data: {}});
      }
  ...
  async _onAudioProvided(ep, cs, evt) {
    ...
    try {
      ...
      const playTask = makeTask(this.logger, {play}, this);
      const {span, ctx} = this.startChildSpan(`nested:${playTask.summary}`);
      playTask.span = span;
      playTask.ctx = ctx;
      playTask.on('playDone', (err) => {
        span.end();
        if (err) this.logger.error({err}, 'Lex:_onAudioProvided Error playing url');
        this.logger.debug('Lex: nested play task completed');
      });

      if (this.events.includes('start-play')) {
        this.performHook(cs, this.eventHook, {event: 'start-play', data: {path: evt.path}});
      }
      await playTask.exec(cs, {ep});
      if (this.events.includes('stop-play')) {
        this.performHook(cs, this.eventHook, {event: 'stop-play', data: {path: evt.path}});
      }
  ...
  _onDtmf(ep, cs, evt) {
    ...
    if (this.events.includes('dtmf')) {
      this.performHook(cs, this.eventHook, {event: 'dtmf', data: evt});
    }
    if (this.passDtmf) {
