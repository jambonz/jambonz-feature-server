...
class TaskConfig extends Task {
  ...
  async exec(cs, {ep} = {}) {
    ...
    if (this.hasDub) {
      if (this.dub.enable) {
        this.logger.info('Config: enabling dubbing of a new audio track');
        cs.startBackgroundTask('dub', this.dub);
      }
      else if (this.dub.enable === false) {
        this.logger.info('Config: disabling dubbing');
        cs.stopBackgroundTask('dub');
      }
    }
  }
