...
class TaskConfig extends Task {
  ...
  get hasAmbientNoise() { return Object.keys(this.ambientNoise).length; }
  ...
  async exec(cs, {ep} = {}) {
    ...
    if (this.hasAmbientNoise) {
      if (this.ambientNoise.enable) {
        this.logger.info('Config: enabling ambientNoise');
        cs.startBackgroundTask('play', {verb: 'play', url: this.ambientNoise.url});
      }
    }
  }
