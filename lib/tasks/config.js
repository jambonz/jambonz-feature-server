...
class TaskConfig extends Task {
  constructor(logger, opts) {
    ...
    if (this.hasSttLatencyCalculation) {
      this.sttLatencyCalculation = !!this.data.sttLatencyCalculation;
    }
  ...
  get hasSttLatencyCalculation() { return Object.keys(this.data).includes('sttLatencyCalculation'); }
  ...
  get summary() {
    ...
    if (this.hasSttLatencyCalculation) phrase.push(
      `sttLatencyCalculation ${this.sttLatencyCalculation ? 'on' : 'off'}`);
  ...
  async exec(cs, {ep} = {}) {
    ...
    if (this.hasSttLatencyCalculation) {
      this.logger.debug(`turning sttLatencyCalculation ${this.sttLatencyCalculation ? 'on' : 'off'}`);
      cs.sttLatencyCalculationEnabled = this.sttLatencyCalculation;
    }
