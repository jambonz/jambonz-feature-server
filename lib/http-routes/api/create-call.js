...
router.post('/', async(req, res) => {
  ...
  try {
    ...
    if (req.body.dual_streams) {
      otherEp = await ms.createEndpoint();
      const sdpParts = otherEp.local.sdp.split('m=');
      const sdpPart2 = 'm=' + sdpParts[1];
      localSdp += sdpPart2;
    }
    ...
    const connectStream = async(remoteSdp) => {
      if (remoteSdp !== sdp) {
        sdp = remoteSdp;
        if (req.body.dual_streams) {
          const sdpParts = remoteSdp.split('m=');
          const sdpPart0 = sdpParts[0];
          const sdpLegA = sdpPart0 + 'm=' + sdpParts[1];
          const sdpLebB = sdpPart0 + 'm=' + sdpParts[2];

          ep.modify(sdpLegA);
          otherEp.modify(sdpLebB);
        } else {
          ep.modify(sdp);
        }
        return true;
