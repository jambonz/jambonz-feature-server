...
const { JAMBONES_DIAL_SBC_FOR_REGISTERED_USER,
  JAMBONES_TRANSCRIBE_EP_DESTROY_DELAY_MS } = require('../../config');
const { configureEP } = require('../../utils/helpers');
      ...
      try {
        const dlg = await srf.createUAC(uri, {...opts,  followRedirects: true, keepUriOnRedirect: true}, {
          ...
          */
            ...
            cs.exec(req);
            // Override endpoint destroy, endpoint stopTranscription methods
            // if JAMBONES_TRANSCRIBE_EP_DESTROY_DELAY_MS is set
            // we need to override here we need cs object to get isCurTaskTranscribe
            if (ep && JAMBONES_TRANSCRIBE_EP_DESTROY_DELAY_MS) {
              configureEP(ep, cs, logger, JAMBONES_TRANSCRIBE_EP_DESTROY_DELAY_MS);
            }
            if (dualEp && JAMBONES_TRANSCRIBE_EP_DESTROY_DELAY_MS) {
              configureEP(dualEp, cs, logger, JAMBONES_TRANSCRIBE_EP_DESTROY_DELAY_MS);
            }
