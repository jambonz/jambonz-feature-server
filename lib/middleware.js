  ...
  async function invokeWebCallback(req, res, next) {
    ...
    try {
      ...
      if (siprec) {
        ...
        if (app.tasks.find((t) => ![TaskName.Config, TaskName.Listen, TaskName.Transcribe].includes(t.name))) {
          logger.info(
            {tasks: app.tasks.map((t) => t.name)},
            'only config, transcribe and/or listen allowed on an incoming siprec call'
          );
          return res.send(503);
        }
      }
