...
const { utilizeJambonzApp } = require('./apps-map');
  ...
  async function retrieveApplication(req, res, next) {
    ...
    try {
      ...
      if (req.locals.queue_name) {
        ...
        app = utilizeJambonzApp('queue', {account_sid, name: req.locals.queue_name});
      } else if (req.locals.called_user) {
        logger.debug(`calling to registered user ${req.locals.called_user}, generating dial app`);
        app = utilizeJambonzApp('user',
          {account_sid, name: req.locals.queue_name, caller_id: req.locals.callingNumber});
