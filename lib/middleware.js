...
const {
  JAMBONES_MYSQL_REFRESH_TTL, JAMBONZ_RECORD_WS_BASE_URL, JAMBONZ_RECORD_WS_USERNAME, JAMBONZ_RECORD_WS_PASSWORD,
} = require('./config');
...
module.exports = function(srf, logger) {
  ...
  async function invokeWebCallback(req, res, next) {
    ...
    const {rootSpan, siprec, application:app, accountInfo} = req.locals;
    let span;
    try {
      ...
      if (accountInfo.account.record_all_calls || app.record_all_calls) {
        // add config verb with nested listen for recording in background
        const config = {
          verb: 'config',
          listen: {
            enable: true,
            url: `${JAMBONZ_RECORD_WS_BASE_URL}/record/${accountInfo.account.bucket_credential.vendor}`,
            wsAuth: {
            ...
            mixType : 'stereo',
            metadata: {
              account_sid: accountInfo.account.account_sid,
              call_sid: req.locals.callSid
            }
          }
        };
        json = [
