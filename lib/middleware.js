...
module.exports = function(srf, logger) {
  ...
  function createRootSpan(req, res, next) {
    ...
    const callIdHex = extCallId.replaceAll('-', '').substring(0, 32);
    const rootSpan = new RootSpan('incoming-call', callIdHex, attributes, tracer, logger);

    const traceId = rootSpan.traceId;
    const {callSid, accountSid} = attributes;
  ...
  async function getAccountDetails(req, res, next) {
    ...
    const childSpan = rootSpan.startChildSpan('lookupAccountDetails');
    try {
      ...
      childSpan.endWithError('Error retrieving account details');
  ...
  async function retrieveApplication(req, res, next) {
    ...
    const childSpan = rootSpan.startChildSpan('lookupApplication');
    try {
      let app;
      ...
      */
      ...
      childSpan.end('Error looking up application');
  ...
  async function invokeWebCallback(req, res, next) {
    ...
    try {
      ...
      if (app.app_json) {
        ...
        childSpan = rootSpan.startChildSpan('performAppWebhook');
      ...
      childSpan?.setAttributes({webhookStatus: err.statusCode});
      childSpan?.endWithError('http error');
      writeAlerts({
