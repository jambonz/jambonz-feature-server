...
module.exports = function(srf, logger) {
  ...
  function createRootSpan(req, res, next) {
    ...
    if (process.env.JAMBONZ_OTEL_ENABLED) logger.info(`createRootSpan: ${rootSpan.traceId}`);
    req.locals = {
      ...req.locals,
      traceId,
      logger: logger.child({callId: req.get('Call-ID'), callSid, traceId}),
      rootSpan
    };
