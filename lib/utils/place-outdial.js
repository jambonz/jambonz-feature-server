...
class SingleDialer extends Emitter {
  ...
  async exec(srf, ms, opts) {
    const childSpan = this.startSpan('placeOutDial:exec', {});
    opts = opts || {};
    ...
    try {
      ...
      inviteSpan = childSpan.startChildSpan('invite', {
        'invite.uri': uri,
      ...
      if (this.killed) {
        ...
        childSpan.endWithError('race condition');
        return;
      }
  ...
  async doAdulting({logger, tasks, application}) {
    ...
    //todo figure out how to grab traceId of original call
    const {sip: {callId}, callSid, linkedSpanId, otel: {tracer}} = this.dlg;
    const rootSpan = new RootSpan('fs:adulting-call', callId, null,
      {linkedSpanId, callId, callSid},
      tracer,
      this.logger);
