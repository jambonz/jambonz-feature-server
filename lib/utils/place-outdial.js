...
class SingleDialer extends Emitter {
  constructor({logger, sbcAddress, target, opts, application, callInfo, accountInfo, rootSpan, span}) {
    super();
    assert(target.type);
...
function placeOutdial({logger, srf, ms, sbcAddress, target, opts, application, callInfo, accountInfo, span}) {
  const myOpts = deepcopy(opts);
  const sd = new SingleDialer({logger, sbcAddress, target, myOpts, application, callInfo, accountInfo, span});
