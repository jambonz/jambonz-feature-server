...
const {
  ...
  JAMBONES_DIAL_TRANSCRIBE_EP_DESTROY_DELAY_MS
} = require('../config');
...
class SingleDialer extends Emitter {
  ...
  _configMsEndpoint() {
    ...
    this.ep.destroy = async() => {
      try {
        if (this.dialTask.transcribeTask && JAMBONES_DIAL_TRANSCRIBE_EP_DESTROY_DELAY_MS) {
          ...
          await new Promise((resolve) => setTimeout(resolve, JAMBONES_DIAL_TRANSCRIBE_EP_DESTROY_DELAY_MS));
        }
        await origDestroy();
      } catch (err) {
