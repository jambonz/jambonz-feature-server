...
const doListen = (logger, app, port, resolve) => {
  const server = app.listen(port, () => {
    ...
    const { ipv4 } = srf.locals;
    const serviceUrl = `http://${ipv4}:${port}`;
    app.locals.srf.locals.serviceUrl = serviceUrl;
    logger.info(`listening for HTTP requests on port ${port}, serviceUrl is ${serviceUrl}`);
    resolve({server, app});
  });
  return server;
};
