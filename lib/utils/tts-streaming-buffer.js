...
class TtsStreamingBuffer extends Emitter {
  ...
  async start() {
    if (this._connectionStatus === TtsStreamingConnectionStatus.Connected) {
      this.logger.info('TtsStreamingBuffer:start already connected');
      // If there is text in the queue, send it immediately.
      // This scenario is when you have multiple gather verbs with nested say stream task
      if (this.queue?.length > 0) {
        // this will send all the tokens that were sent during the initialization
        this.logger.info('TtsStreamingBuffer:start queue has tokens, sending immediately');
        this._feedQueue();
      }
      return;
    }
