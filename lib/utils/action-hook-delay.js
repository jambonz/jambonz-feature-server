...
class ActionHookDelayProcessor extends Emitter {
  ...
  start() {
    ...
    if (this.noResponseTimeout > 0) {
      ...
      this.logger.debug('ActionHookDelayProcessor#start: noResponseTimeout is 0 not calling _onNoResponseTimer');
    }
  ...
  async _onNoResponseGiveUpTimer() {
    this._active = false;
    if (!this.giveUpActions) {
      ...
      this.logger.info('ActionHookDelayProcessor#_onNoResponseGiveUpTimer executing giveUpActions');
      const giveUpTasks = normalizeJambones(this.logger, this.giveUpActions);
      for (const t of giveUpTasks) {
        this._taskInProgress = makeTask(this.logger, t);
        this._taskInProgress.disableTracing = true;
        await this._taskInProgress.exec(this.cs, {ep: this.ep});
      }
    }
  }
}
