...
class ActionHookDelayProcessor extends Emitter {
  constructor(logger, opts, cs, ep) {
    ...
    const disabled = this.init(opts);
    if (!disabled && (!this.actions || !Array.isArray(this.actions) || this.actions.length === 0)) {
      assert('ActionHookDelayProcessor: no actions specified');
    }
    else if (disabled && this.actions.some((a) => !a.verb || !['play', 'say'].includes(a.verb))) {
      assert(`ActionHookDelayProcessor: invalid actions specified: ${JSON.stringify(this.actions)}`);
    }
  }
