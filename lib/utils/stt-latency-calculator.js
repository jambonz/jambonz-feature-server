...
class SttLatencyCalculator extends Emitter {
  ...
  calculateLatency() {
    ...
    const stt_on_transcription_time = this.cs.stt_on_transcription_time || stt_stop_time;
    const start_talking_time = this.talkspurts[0]?.start;
    let lastIdx = this.talkspurts.length - 1;
    lastIdx = lastIdx < 0 ? 0 : lastIdx;
    const stop_talking_time = this.talkspurts[lastIdx]?.[incompleteTalkspurt ? 'start' : 'stop'];
    ...
    return {
      ...
      stt_latency: parseFloat((stt_on_transcription_time - stop_talking_time) / 1000).toFixed(2),
