const {getGlobalDispatcher, setGlobalDispatcher, Dispatcher, ProxyAgent, Client, Pool} = require('undici');
...
const defaultDispatcher = HTTP_PROXY_IP ? new ProxyAgent(`${HTTP_PROXY_IP}${HTTP_PROXY_PORT ? `:${HTTP_PROXY_PORT}` : ''}`) : getGlobalDispatcher();
