...
class HttpRequestor extends BaseRequestor {
  ...
  async request(type, hook, params, httpHeaders = {}) {
    ...
    try {
      ...
      // Retry count: rc valid values: 1-5, default is 1
      // rc is the number of attempts we'll make AFTER the initial try
      const rc = parseInt(hashObj.rc) || 1;
      ...
      const requestFunc = HTTP_PROXY_IP ? request : client.request.bind(client);
      ...
      const makeRequest = async() => {
        ...
        const {statusCode, headers, body} = await requestFunc(
          this.baseUrl,
          requestOptions
        );
