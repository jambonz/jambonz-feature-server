...
class BackgroundTaskManager extends Emitter {
  ...
  async newTask(type, opts) {
    ...
    switch (type) {
      ...
      case 'transcribe':
        ...
        break;
      case 'ambient-noise':
        await this._initAmbientNoise(opts);
        break;
      default:
        break;
    }
  ...
  async _initAmbientNoise(opts) {
    const {enable, url} = opts;
    const {ep} = this.cs;
    this.logger.debug({enable, url}, 'BackgroundTaskManager:_initAmbientNoise');
    try {
      if (!ep) throw new Error('no endpoint exists for this call');
      await ep.api('uuid_an', enable ? `start ${url}` : 'stop');
    } catch (err) {
      this.logger.info({err, opts}, 'BackgroundTaskManager:_initAmbientNoise - Error managing ambient noise');
    }
  }
