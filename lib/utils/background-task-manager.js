...
class BackgroundTaskManager extends Emitter {
  ...
  stop(type) {
    ...
    if (task) {
      ...
      const {ep} = this.cs;
      if (ep) {
        ep.api('uuid_dub', [ep.uuid, 'stop']).catch((err) => {this.logger.error(err, 'Error stopping dub');});
      }
    }
    else {
  ...
  async _initDubbing(opts) {
    const {enable, play, loop = false} = opts;
    const {ep} = this.cs;
    this.logger.debug({enable, play}, 'BackgroundTaskManager:_initDubbing');
    try {
      if (!ep) throw new Error('no endpoint exists for this call');
      if (!('enable' in opts)) throw new Error('BackgroundTaskManager:_initDubbing - missing enable parameter');
      if (enable) {
        if (!play) throw new Error('BackgroundTaskManager:_initDubbing - missing play parameter');
        if (!play.endsWith('.mp3')) {
          throw new Error('BackgroundTaskManager:_initDubbing - only mp3 files are supported');
        }
        if (!play.startsWith('http')) {
          throw new Error('BackgroundTaskManager:_initDubbing - play param must be an http(s) urls to mp3 audio');
        }
      }
      const args = [ep.uuid, enable ? 'play' : 'stop'];
      if (play) args.push(play);
      if (loop) args.push('loop');
      await ep.api('uuid_dub', args);
    } catch (err) {
      this.logger.info({err, opts}, 'BackgroundTaskManager:_initDubbing - Error managing ambient noise');
    }
  }
