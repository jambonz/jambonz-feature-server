...
class BackgroundTaskManager extends Emitter {
  ...
  async newTask(type, taskOpts) {
    ...
    switch (type) {
      ...
      case 'transcribe':
        ...
        break;
      case 'play':
        task = await this._initPlay(taskOpts);
        break;
      default:
        break;
    }
  ...
  async _initPlay(opts) {
    let task;
    try {
      const t = normalizeJambones(this.logger, [opts]);
      task = makeTask(this.logger, t[0]);
      const resources = await this.cs._evaluatePreconditions(task);
      const {span, ctx} = this.rootSpan.startChildSpan(`background-play:${task.summary}`);
      task.span = span;
      task.ctx = ctx;
      task.bugname_prefix = 'background_play_';
      task.exec(this.cs, resources)
        .then(this._taskCompleted.bind(this, 'play', task))
        .catch(this._taskError.bind(this, 'play', task));
    } catch (err) {
      this.logger.info(err, 'BackgroundTaskManager:_initPlay - Error creating play task');
    }
    return task;
  }
