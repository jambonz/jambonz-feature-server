...
const compileDeepgramTranscripts = (bufferedTranscripts, channel, language) => {
  ...
  const finalTranscript = bufferedTranscripts.reduce((acc, evt) => {
    ...
    /* If new transcript consists of only digits with a trailing comma or period */
    if (newTranscript.match(/^\d+[,.]$/)) {
      newTranscript = newTranscript.slice(0, -1);  // Remove the last character
    }
