...
const compileDeepgramTranscripts = (bufferedTranscripts, channel, language) => {
  let totalConfidence = 0;
  const finalTranscript = bufferedTranscripts.reduce((acc, evt) => {
    totalConfidence += evt.alternatives[0].confidence;
    acc.alternatives[0].transcript += ` ${evt.alternatives[0].transcript}`;
  }, {
  ...
  finalTranscript.alternatives[0].confidence = parseFloat(totalConfidence / bufferedTranscripts.length).toFixed(2);
