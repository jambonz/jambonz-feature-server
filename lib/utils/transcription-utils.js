...
module.exports = (logger) => {
  ...
  const setChannelVarsForStt = (task, sttCredentials, language, rOpts = {}) => {
    ...
    if ('google' === vendor) {
      ...
      const model = openaiOptions.model || rOpts.model || 'whisper-1';
      ...
      /* special case for hints: they need to be presented in a prompt */
      if (rOpts.hints?.length > 0 && !opts.prompt) {
        let prompt;
        const fragment = rOpts.hints.map((h) => h.trim()).join(', ');
        if (model === 'whisper-1') {
          /* limited to 224 tokens, provide a simple list */
          prompt = fragment;
        }
        else {
          /* provide a more complex prompt */
          prompt = `Please spell the following words correctly: ${fragment}.`;
        }
        opts.OPENAI_PROMPT = prompt;
      }
    }
