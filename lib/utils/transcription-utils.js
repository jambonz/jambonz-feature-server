  ...
  const setChannelVarsForStt = (task, sttCredentials, rOpts = {}) => {
    ...
    if ('google' === vendor) {
      ...
      options = {
        ...options,
        ...(rOpts.hints.length > 0 && typeof rOpts.hints[0] === 'string' &&
        {JAMBONZ_SPEECH_HINTS: rOpts.hints.join(',')}),
        ...(rOpts.hints.length > 0 && typeof rOpts.hints[0] === 'object' &&
        {JAMBONZ_SPEECH_HINTS: JSON.stringify(rOpts.hints)}),
        ...(typeof rOpts.hintsBoost === 'number' && {JAMBONZ_SPEECH_HINTS_BOOST: rOpts.hintsBoost})
      };
