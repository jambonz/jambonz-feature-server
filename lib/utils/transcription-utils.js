...
const normalizeAmiVoice = (evt, channel, language) => {
  const copy = JSON.parse(JSON.stringify(evt));
  
  // AMI Voice API provides results in a specific format
  // Based on the AMI Voice API documentation, responses include:
  // - results array with text and confidence
  // - is_final indicating if this is a final result
  const is_final = evt.is_final || evt.type === 'final' || false;
  const confidence = evt.confidence || (evt.results && evt.results[0]?.confidence) || 1.0;
  const transcript = evt.text || (evt.results && evt.results[0]?.text) || '';
...
module.exports = (logger) => {
  const normalizeTranscription = (evt, vendor, channel, language, shortUtterance, punctuation) => {
    ...
    switch (vendor) {
      ...
      case 'amivoice':
        return normalizeAmiVoice(evt, channel, language);
      case 'voxist':
  ...
  const setChannelVarsForStt = (task, sttCredentials, language, rOpts = {}) => {
    ...
    if ('google' === vendor) {
      ...
      opts = {
        ...opts,
        ...(sttCredentials.api_key) &&
          {AMIVOICE_API_KEY: sttCredentials.api_key},
        ...(sttCredentials.model_id && {AMIVOICE_MODEL_ID: sttCredentials.model_id}),
        ...(language && {AMIVOICE_LANGUAGE: language}),
        ...(rOpts.hints?.length > 0 && typeof rOpts.hints[0] === 'string' &&
          {AMIVOICE_HINTS: rOpts.hints.join(',')}),
        ...(rOpts.hints?.length > 0 && typeof rOpts.hints[0] === 'object' &&
          {AMIVOICE_HINTS: JSON.stringify(rOpts.hints)})
      };
      }
