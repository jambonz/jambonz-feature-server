...
const stickyVars = {
  ...
  nvidia: [
    'NVIDIA_PROFANITY_FILTER',
    'NVIDIA_PUNCTUATION',
    'NVIDIA_WORD_TIME_OFFSETS',
    'NVIDIA_SPEAKER_DIARIZATION',
    'NVIDIA_SEPARATE_RECOGNITION_PER_CHANNEL',
    'NVIDIA_HINTS'
  ]
};
...
module.exports = (logger) => {
  ...
  const setChannelVarsForStt = (task, sttCredentials, rOpts = {}) => {
    ...
    if ('google' === rOpts.vendor) {
      ...
      const {nvOptions = {}} = rOpts;
      opts = {
        ...opts,
        ...((nvOptions.profanityFilter || rOpts.profanityFilter) && {NVIDIA_PROFANITY_FILTER: 1}),
        ...((nvOptions.punctuation || rOpts.punctuation) && {NVIDIA_PUNCTUATION: 1}),
        ...((rOpts.words || nvOptions.wordTimeOffset) && {NVIDIA_WORD_TIME_OFFSETS: 1}),
        ...(nvOptions.maxAlternatives && {NVIDIA_MAX_ALTERNATIVES: nvOptions.maxAlternatives}),
        ...(rOpts.model && {NVIDIA_MODEL: rOpts.model}),
        ...(nvOptions.rivaUri && {NVIDIA_RIVA_URI: nvOptions.rivaUri}),
        ...(nvOptions.verbatimTranscripts && {NVIDIA_VERBATIM_TRANSCRIPTS: 1}),
