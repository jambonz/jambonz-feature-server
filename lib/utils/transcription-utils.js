...
const compileSonioxTranscripts = (arr, channel, language) => {
  const words = arr.map((m) => m.vendor.evt.words).flat();
...
const normalizeSoniox = (evt, channel, language) => {
  ...
  /* remove <end> token, if there (signals speech endpoint) */
  const lastWord = evt.words.pop();
  const endpointReached = lastWord && lastWord.text === '<end>';
  if (!endpointReached) evt.words.push(lastWord);

  const is_final = endpointReached /*&& evt.words.filter((word) => word.is_final).length === evt.words.length */;
  const transcript = evt.words.reduce((acc, word) => {
    if ([',', '.', '?', '!'].includes(word.text)) return `${acc}${word.text}`;
    else return `${acc} ${word.text}`;
  }, '').trim();
  const confidence = evt.words.reduce((acc, word) => acc + word.confidence, 0) / evt.words.length;
  const alternatives = [{transcript, confidence}];
  return {
    language_code: language,
    channel_tag: channel,
