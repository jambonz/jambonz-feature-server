...
const normalizeHoundify = (evt, channel, language) => {
  const copy = JSON.parse(JSON.stringify(evt));
  console.log('Houndify event:', copy);
  const alternatives = [];
  let is_final = false;
  if (evt.type && evt.type === 'Turn') {
    // v3 is here
...
module.exports = (logger) => {
  ...
  const setChannelVarsForStt = (task, sttCredentials, language, rOpts = {}) => {
    ...
    if ('google' === vendor) {
      ...
      const {latitude, longitude, city, state, country, timeZone, domain, audioEndpoint,
      ...
      opts = {
        ...
        HOUNDIFY_MAX_SILENCE_SECONDS: MaxSilenceSeconds || 1.5,
        HOUNDIFY_MAX_SILENCE_AFTER_FULL_QUERY_SECONDS: MaxSilenceAfterFullQuerySeconds || 0.5,
