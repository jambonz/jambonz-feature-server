...
const configureEP = (ep, cs, logger, delayMs, ignoreCurrentTaskCheck = false) => {
  ...
  ep.stopTranscription = async(...args) => {
    // when coming from stopTranscription in fallback scenario or pause, we should stop transcription immediately
    const isImmediate = args && args[0] ? args[0].immediate : false;
    if (isImmediate) {
      return await origStopTranscription(...args);
    }
    // if not coming from fallback scenario, or coming from place-outdial we should wait for the delayMs
    if (ignoreCurrentTaskCheck || cs?.isCurTaskTranscribe) {
      logger.debug('configureEP: ' +
