...
class WsRequestor extends BaseRequestor {
  constructor(logger, account_sid, hook, secret) {
    ...
    // Parse URL hash for retry configuration
    const parsedUrl = parseUrl(this.url);
    const hash = parsedUrl.hash || '';
    const hashObj = hash ? this._parseHashParams(hash) : {};

    // Store clean URL (without hash) for WebSocket connection
      ...
      try {
        ...
        while (retryCount <= this.maxReconnects) {
          try {
            ...
            lastError = null; // Clear lastError on success
            ...
            // If we get here, we've either exhausted retries or the error isn't retryable
  ...
  _onError(err) {
    if (this.connections > 0) {
      this.logger.info({url: this.url, err, retryPolicy: this.retryPolicy}, 'WsRequestor:_onError');

      // Check if this error should be retried based on the retry policy
      if (this.retryPolicyValues?.length && this._shouldRetry(err, this.retryPolicyValues)) {
        this.logger.info({url: this.url}, 'WsRequestor:_onError - will attempt retry based on retry policy');
        // The _onSocketClosed handler will handle the reconnection
      }
    }

    // Always emit not-ready for connection errors during initial connection attempts
    // This ensures that the Promise in _connect() gets properly rejected
    if (this.connectInProgress) {
  ...
  _onUnexpectedResponse(ws, req, res) {
    ...
    // Reset connectInProgress since this connection attempt failed
    this.connectInProgress = false;

    // Emit not-ready to properly reject the _connect() Promise
  ...
  _onSocketClosed() {
    ...
    // For socket closure after established connection, only reconnect if not gracefully closed
    // and we haven't exceeded max reconnects for the session
  ...
  /**
   * Shared method to schedule reconnection with exponential backoff
   * @param {string} source - Source method name for logging
   */
