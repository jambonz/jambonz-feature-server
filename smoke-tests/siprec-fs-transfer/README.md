...
1. answers an inbound call, starts SIPREC recording, and puts the call in a queue
...
| `SMOKE_AGENT_SIP_URI` | yes* | - | agent target, e.g. `sip:agent@10.0.0.50` |
| `SMOKE_AGENT_TO_JSON` | yes* | - | full `to` object instead, e.g. `{"type":"phone","number":"+15551234567","trunk":"my-carrier"}` |
...
\* one of `SMOKE_AGENT_SIP_URI` or `SMOKE_AGENT_TO_JSON`.
...
SMOKE_AGENT_SIP_URI=sip:agent@10.0.0.50 \
node index.js
```

It prints the hook URLs, then waits. Point an application's call hook at
`http://<advertise>/inbound`, place one inbound call to it with audio, and leave it up -
the script drives everything else and exits 0 on PASS, 1 on FAIL, 2 if it could not run
the scenario at all.
